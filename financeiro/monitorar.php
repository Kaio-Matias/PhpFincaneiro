...
<table width="770" border="0" align="center">
  <tr> 
    <td colspan=8 align="center"><b>Monitorar</b></td>
  </tr>
  <tr> <form name="adicionar" method="post" action="monitorar.php">
    <td align=center>Mês: <input type=text name=mes size=3 value="<?=$mes?>"> Ano: <input type=text name=ano size=5 value="<?=$ano?>"> <input type=submit name=botao size=5 value="Enviar"><br><br></td>
  </tr>
  <tr>
   ...
   <td align="center"  class="tdfundo">
	  <table width="100%" border="0">
        <tr class="tdcabecalho1"> 
          ...
          <td align="center"><font size="0"><b>%Ad. Valorem</b></td>
          <td align="center"><font size="0"><b>Desp. Tarifas</b></td>
          <td align="center"><font size="0"><b>TED</b></td>
          <td align="center"><font size="0"><b>Cartório</b></td>
          <td align="center"><font size="0"><b>Desc. Com.</b></td>
          <td align="center"><font size="0"><b>Mora</b></td>
          <td align="center"><font size="0"><b>IOF</b></td>
          <td align="center"><font size="0"><b>CPMF</b></td>
...
$result = execsql("SELECT a.idfactoring, b.nome, valbruto, jurosoperacao, advalorem, prazomedio, desptarifas, ted, cartorio, desccom
			, jurosmora, iof, cpmf, jurosprorrogacao, recompra, valretido,   DATE_FORMAT(dataoperacao,'%d/%m/%Y') 
	from $mysql_entradas_table a, $mysql_factoring_table b where b.idfactoring = a.idfactoring and mes = '$mes' and ano = '$ano'  order by idfactoring, dataoperacao");
while($row = mysql_fetch_row($result)){

	if ($idfactoring != $row[0] && $idfactoring != "") { 
		$prazomedio = number_format($prazomedio/$valbruto,'0');
		echo "
        <tr class=\"tdsubcabecalho1\"  id=\"$idfactoring\"> 
          ...
          <td align=\"center\"><font size=\"1\">".$prazomedio."</td>
	    </tr>";
		echo $factoring;
		$factoring = ""; 
		$valbruto = 0; $jurosoperacao = 0; $advalorem = 0; $prazomedio = 0; $desptarifas = 0; $ted = 0; $cartorio = 0; $desccom = 0;
		$jurosmora = 0; $iof = 0; $cpmf = 0; $prorrogacao = 0; $recompra = 0; $valretido = 0; $valliquido = 0;
	}
	...
	$factoring .= "
		...
		$factoring .= "  id=\"$row[0]-$i\">
          ...
          <td align=\"right\"><font size=\"1\">".@number_format(($row[4]/$row[2])*100,'3',',','.')."</td>
          ...
          <td align=\"right\"><font size=\"1\">".$row[5]."</td>
	    </tr>";
	 $i++;
	 $idfactoring = $row[0];
	 $nomfactoring = $row[1];
	 ...
	 $tvalliquido += $row[2]-$row[3]-$row[4]-$row[6]-$row[7]-$row[8]-$row[9]-$row[10]-$row[11]-$row[12]-$row[13]-$row[14]-$row[15];
        ...
        <tr class=\"tdsubcabecalho1\" id=\"$idfactoring\"> 
 		...
 		$tprazomedio = @number_format($tprazomedio/$tvalbruto,'0');
		echo "
        <tr class=\"tdcabecalho1\"> 
          ...
          <td align=\"center\"><font size=\"1\">".$tprazomedio."</td>
	    </tr>";
