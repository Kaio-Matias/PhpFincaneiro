...
require "../../common/config.financeiro.php";
require "../../common/common.php";
require "../../common/common.financeiro.php";
...
if($campo == "motivorecusa") {
	...
	$sql = "SELECT codcliente, nome FROM $mysql_clientes_table where codfilial $filiais and codgrpcli = '65' GROUP BY codcliente order by codcliente";
} elseif($campo == "loja") {
	$sql = "SELECT loja FROM $mysql_lojas_table order by loja";
	$sem = "1";
} elseif($campo == "idrede") {
	$sql = "SELECT idrede, nome FROM $mysql_redes_table order by idrede";
} elseif($campo == "notafiscal") {
	$sql = "SELECT loja FROM $mysql_lojas_table order by loja";
}

if ($campo != "notafiscal" && $campo != "codcliente") { 
	?>
...
<?
if ($campo != "codcliente" && $campo != "notafiscal") { 
	?>
