...
echo '
  <tr '.$cores[tdsubcabecalho1].' id="CA'.$row[2].'">
	...
	';
	$i = 0;
	$result3 = execsql("SELECT sum(a.valorbruto+a.valordesconto+a.valoradicional), b.nome, a.condpag from $mysql_resumoprazo_table a, $mysql_condpg_table b where a.condpag = b.codcondicaopg and codcanal = '$row[2]' and $where group by a.condpag");
	while($row3 = mysql_fetch_row($result3)) {
	if ($i % 2) { $cor = $cores['tddetalhe1'];} else { $cor = $cores['tdfundo']; }	
echo '
  <tr '.$cor.' id="CA'.$row[2].'-MP'.$row3[2].'0">
	<td nowrap align="left"><font size="0">'.$row3[2].'-'.$row3[1].'</td>';
		$n = 0;
		while (list ($chave, $prazo) = each ($prazos)) {
			$result2 = execsql("SELECT sum(a.valorbruto+a.valordesconto+a.valoradicional) from $mysql_resumoprazo_table a where $where and a.codcanal = '$row[2]' and a.condpag = '$row3[2]' and $prazo");
			$row2 = mysql_fetch_row($result2);	
			echo '
...
if (isset($codcanal)) {
	...
	if (isset($codmeiopg)) {
		echo " e cond. de pagamento: <b>$codmeiopg</b>";
	} 

	$where2 = str_replace("data","datafatura",$where);
	$where2 = str_replace("codcanal","codgrpcliente",$where2);
	if (isset($codmeiopg)) $where2 .= " and codcondicaopg = '$codmeiopg' ";
	...
	where $where2 and codgrpcliente = '$codcanal' and ".$prazos[$u]." group by documento";
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)) { 
