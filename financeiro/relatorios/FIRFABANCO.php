...
$result = execsql("SELECT a.codmeiopg, b.nome $select from $mysql_vendas_table a
					...
					where codtipofatura $bonificacao and codtipofatura in ('ZANT','EANT','EVEF','ZDAI','EDAI') and $where3 and entregue in ('','N') group by codmeiopg");
//,'ZDAI','ZDAC','EDAI','EDAC'
while($row = mysql_fetch_row($result)) { $i= 2;
echo '
...
//,'ZDAI','ZDAC','EDAI','EDAC'
	...
	<table width="99%" border="0" align="center" cellpadding="2" cellspacing="1">
		<tr class=tdcabecalho>
			...
			<td align="center"><font color="#FFFFFF" size="0">Vend</td>
			<td align="center"><font color="#FFFFFF" size="0">MP</td>
...
//	$sql = "select  documento, codtipofatura, notafiscal, codcliente, codfilial, DATE_FORMAT(datafatura,'%d/%m/%Y'), codvendedor, sum(valorbruto+valordesconto+valoradicional) from $mysql_vendas_table a where $where2 and codmeiopg = '$codmeiopg' and ".$query[$u-2]." group by documento";
	...
	$sql = "select  documento, codtipofatura, notafiscal, codcliente, codfilial, DATE_FORMAT(datafatura,'%d/%m/%Y'), codvendedor, sum(valorbruto+valordesconto+valoradicional), codmeiopg from $mysql_vendas_table a where codtipofatura $bonificacao and $where2 and codmeiopg = '$codmeiopg' and ".$query[$u-2]." group by documento";
