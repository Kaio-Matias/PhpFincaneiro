...
foreach ($matriz as $cod => $mess) {
	...
	while($row = mysql_fetch_row($result)){
		...
		if($num_rows != 0){
			$sql = "UPDATE resumogerotinatmp SET volreal = '$row[5]', prcreal = '$row[6]', valliquido = '$row[7]', valimpostos = '$row[8]', valcusto = '$row[9]' WHERE semana = '$semana' and ano = '$ano' and mes = '$mes' and codfilial = '$row[0]' and codcanal = '$row[1]' and codproduto = '$row[3]' and codvendedor = '$row[4]'";
			db_query($sql);
			$regcmsm[$semana]++;
		} else {
...
db_query("INSERT INTO atualizacao (data,sistema) VALUES ('".date("Y-m-d H:i:s")."','GVENDAS')");


mail("portal@valedourado.com.br", "Gestão de Vendas - Vendas sem meta", $conteudo, "From: GVENDAS");
?>
