...
$pedidos = db_query("select datapedido, docsd, codproduto,qntordem , comissao, valoricms, valoricmssub, valoripi, valordesconto, motivorecusa  from gvendas.pedidos where datapedido >= $data1 and codtipofatura $bonificacao order by docsd");

while($det = mysql_fetch_array($pedidos)) {
	  $vlped = $det[4] + $det[5] + $det[6] + $det[7];
  	  db_query("INSERT INTO logistica.atendimento_produto VALUES ('$det[0]','$det[1]','$det[2]','$det[3]','$vlped','','0000-00-00','0000-00-00','$det[9]')");
...
// ------------------------------------------------------- Processa atendimento
    $row2 = mysql_fetch_array(db_query("select sum(valoricms+valoricmssub+valoripi) from gvendas.pedidos where docsd = '$row[0]' group by docsd"));
    $valorpedido = $row2[0] - $row[1];
