...
$sql = "select a.codproduto, sum(quantidade_produto), lote 
		...
		where data_entrada ='20080112' and excluido='N'  group by a.codproduto, lote";
$result = db_query($sql);
$email .= "Número de entradas no estoque LIVRE: ".mysql_num_rows($result)."\n";

if(mysql_num_rows($result)>0) {
	$inseriu = 1;
	$arquivo = "H52110060001PANO"."12012008".str_pad($r[2],6,"0",STR_PAD_LEFT)."
";
...
";
	...
	$myfile = fopen($CFG->diretorio."estoque/".$hoje.".jam","a");
...
if(mysql_num_rows($result)>0) {
	$inseriu = 1;
	$arquivo = "H52510060001PANO"."12012008".str_pad($r[2],6,"0",STR_PAD_LEFT)."
";
	$email .= str_pad("PRODUTO",10," ",STR_PAD_RIGHT).str_pad("QUANT",10," ",STR_PAD_RIGHT)."LOTE \n";
	while($row = mysql_fetch_row($result)){
		$arquivo .= "D".str_pad($row[0], 10, " ", STR_PAD_RIGHT).
...
mail("james.reig@valedourado.com.br", "Armazém Dinâmico - ENTRADA PRODUÇÃO", $email, "From: GPRODUCAO");
