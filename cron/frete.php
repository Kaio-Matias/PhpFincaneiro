...
while($row = mysql_fetch_row($result)){
	...
	$sql = "update prestconta.romfrete set tipofatura='$row2[1]' where notafiscal = '$row[1]' and romaneio = '$row[0]'";
	db_query($sql);
}
...
while($row = mysql_fetch_row($result)){
	...
	$row2 = mysql_fetch_row($result2);
	if ($row2[0] != $row[3]) {
		$sql = "update prestconta.romfrete set codfilial='$row2[0]' where notafiscal = '$row[1]'  and romaneio = '$row[0]'";
...
SELECT a.codfilial, b.origem, a.codgrpcliente, a.codcliente, b.romaneio, a.docsd, a.notafiscal, b.classificacao, b.transportador, c.cidade, a.uf, a.datapedido, a.dataremessa, a.datafatura, b.datasaida, b.datacanhoto
...
where datafatura >= '$de' and datafatura <= '$ate' and b.codcliente = a.codcliente and b.notafiscal = a.notafiscal  group by a.notafiscal, a.centro, a.codcliente";
db_query($sql);
...
//mail("james.reig@valedourado.com.br", "Gestão de Logística - Resumo Frete", $conteudo, "From: LOGISTICA");
?>
