...
if (file_exists($CFG->diretorio."pedidos.txt")) {
$fd = fopen ($CFG->diretorio.'pedidos.txt', "r");
	$conteudo .= "\nArquivo Encontrado!";
	$conteudo .= "\nDeletando Pedidos...";
    db_query("DELETE FROM gvendas.pedidos where mes = '".$mesc."' and ano = '".$anoc."'");
	$conteudo .= "\nPedidos Deletada.";
	$conteudo .= "\n\nInserindo no banco...";
while (!feof ($fd)) {
	$i++;
...
$myfile = fopen($CFG->log."PEDIDOS.TXT","w");
$fp = fwrite($myfile,$conteudo);
fclose($myfile);


db_query("DELETE FROM resumopedido where mes = '".$mesc."' and ano = '".$anoc."'");
db_query("INSERT INTO resumopedido SELECT datapedido, mes, ano, codfilial, codgrpcliente, codproduto, motivorecusa, sum(qntordem-qntconfirmada), sum(valorbruto), sum(valordesconto), sum(valoricms+valoricmssub+valoripi+valorpis+valorcofins) FROM pedidos where mes = '$mesc' and ano = '$anoc' GROUP BY codfilial, codgrpcliente, codproduto, motivorecusa, datapedido");

mail("saulo.cavalcante@valedourado.com.br", "Gestão de Vendas - Pedidos", $email, "From: GVENDAS");
