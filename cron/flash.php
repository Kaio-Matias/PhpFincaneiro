...
$rvendedor = db_query("select codvendedor, cx from gvendas.vendedorcx order by codvendedor");
while($vendedor = mysql_fetch_row($rvendedor)){
	...
	while($row = mysql_fetch_row($result)){
		...
		$mensagem .= "
		  ...
		  <tr> 
		   <td colspan=\"4\" nowrap bgcolor=\"#C7C7C7\" align=\"center\"><font size=\"-2\"><b>% Vol.</b></td>
		   <td colspan=\"5\" nowrap bgcolor=\"#C7C7C7\" align=\"center\"><font size=\"-2\"><b>% R$</b></td>
		  </tr>
		  <tr>
		   <td colspan=\"4\" nowrap align=\"right\"><font size=\"-2\">".@number_format(($row2[0]*100)/$row[0],'2',',','.')."</td>
		   <td colspan=\"5\" nowrap align=\"right\"><font size=\"-2\">".number_format(($row2[1]*$row2[0]*100)/$row[1]*$row[0],'2',',','.')."</td>
		  </tr>";
	}
	...
	//mail("portal@valedourado.com.br", $codvendedor, $msg_body, "From: ".$vendedor[1]."\n".$mailheaders);  
	//mail("ilpisa105@emvia.com.br", $codvendedor, $msg_body, "From: ".Mostrarcx($codvendedor)."\n".$mailheaders);  
}

mail("portal@valedourado.com.br", "Envio de Flash", $admin, "From: GVENDAS");  
