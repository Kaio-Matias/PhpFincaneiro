...
if(isset($search) || isset($s)){
	if($stmt == ''){
		$sql = "select a.numero, a.tribunal, a.codtipoacao, a.datprocesso, a.codprocesso from $mysql_processos_table a where (";
	}
	else{
 ...
 //   $stmt = eregi_replace("T_B", '(', $stmt);
	...
	if(!isset($query) || $query == ''){
		...
		if(isset($parte) && $parte != ''){
			    $sql = "select a.numero, a.tribunal, a.codtipoacao, a.datprocesso, a.codprocesso from $mysql_processos_table a, $mysql_pessoas_table b, $mysql_processopartes_table c, $mysql_processopatronos_table d  where (";
	...
	echo "<form method=post> <br>
 	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=0>			<TR>
			<TD>
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=0>
					<TR>
					<TD colspan=100% align=middle><B>SAC - Listar Processos			</td>
						</TR>
		</table>	</td>
			</tr>
		</table><br><br> ";
...
<TR>
					<TD class=tdcabecalho1 colspan=100% align=left><B>Para visualizar todos os processos, clique em avançar sem preencher os campos.			</td>
						...
						<TR>
						<TD class=tdsubcabecalho1 align=right width=27%>Pessoas: </td>
						<td class=back>
							<input type=text name=parte>
						</td>
						</tr>
						<TR>
						<TD class=tdsubcabecalho1 align=right width=27%>Patrono: </td>
						<td class=back>
							<input type=text name=patrono>
						</td>
						</tr>
						<TR>
