...
if ($mostra == '1'){
    $mostra = '';
  include "../common/data.php";
  echo "<form action=receitas.php name=incdes method=post>";
  ?>
   <table width="1200" border="1" align="center" cellpadding="0" cellspacing="0">  
  <?
  echo '
	<TR> 
	   	<td align="right" class="tdsubcabecalho1" width="30%">Grupo Receitas:</td>
	    <td width=50% class=back><select name=grupo1>';createSelectGrupopp();echo '</select></td>
...
if ($inext != '') {
  ...
  echo '</tr>	
       <tr class="tdsubcabecalho2">
				<TD colspan=100% align=middle><B>Receitas</td>
     </tr>	
  ...
  echo '<tr class="tddetalhe">
	    <td align="center"><font size="2">Receita</font></td>
		<td align="center"><font size="2">Valor</font></td>
		<td align="center"><font size="2">Dt.Crédito</font></td>
	  </tr>';

  $result = execsql("select nome, sum(val_real), d.grupop, data, centro from $mysql_previsao_table as d
                   inner join  $mysql_grupop_table as g ON g.grupop = d.grupop
				   where $selgrupo data >= ".$data1." and data <= ".$data2." group by d.grupop");
  while($row = mysql_fetch_row($result)){
	echo '
     ...
     $result2 = execsql("select descricao, val_real, DATE_FORMAT(data,'%d/%m/%Y'), d.grupop from $mysql_previsao_table as d
                   inner join  $mysql_grupop_table as g ON g.grupop = d.grupop
				   where $selgrupo data >= ".$data1." and data <= ".$data2." and d.grupop = ".$row[2]." order by data");
