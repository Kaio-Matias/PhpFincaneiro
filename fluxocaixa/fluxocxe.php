...
if ($avancar == 'SIM')
    ...
    <table width="350" border="0" align="center" cellpadding="2" cellspacing="1">
     ...
     <tr class="tddetalhe1"> 
      <td align="center"><a href="fluxocxe.php">Lançar Novo Fluxo Caixa</a></center></td>
     </tr>
     <tr class="tddetalhe1"> 
	...
	<?
	execsql("DELETE FROM $mysql_fluxocxe_table WHERE mes='".$mes."' and ano='".$ano."' and centro='".$centro."'");
	...
	while($row = mysql_fetch_row($result)){
	    ...
	    $sql = execsql("insert into $mysql_fluxocxe_table values (".$centro.",'0',".$row[0].",".$mes.",".$ano.",".valor_banco($$preco).",".valor_banco($$preco2).")");
	}
		exit;
	}

?>
<form name="form1" method="post" action="fluxocxe.php">
  <p align="center"><font size = "3">Digite a previsão de fluxo de caixa - Entrada.</font></p>
...
<br>
   <table width="500" border="0" align="center" cellpadding="0" cellspacing="1">
	...
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)){
    	$prev = mysql_fetch_row(execsql("select valor, valorr from $mysql_fluxocxe_table WHERE grupop = '".$row[0]."' and mes='".$mes."' and ano='".$ano."' and centro='".$centro."'"));
	...
	?>
          <tr> 
            <td colspan="2" align="left"><br><a href="fluxocxe.php"><img src="images/btvoltar.gif"  border="0"></a></td>
...
<?
	$i = 1;
	$sql = "select * from $mysql_fluxocxe_table WHERE mes='".$mes."' and ano='".$ano."' and centro='".$centro."' order by centro, grupop";
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)){
	if ($i % 2) { $cor = 'class=tddetalhe1';} else { $cor = '';}
	$preco = $row[0]."preco";
...
<table width="580" border="0" align="center">
  <tr> 
    <td align="center" class="tdcabecalho"><b>Previsão de Fluxo de Caixa - Entradas</b></td>
  </tr>
  <tr>
