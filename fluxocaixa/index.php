...
$hora = date('Hi');
/* api
$data = simplexml_load_file("http://conteudo.bcb.gov.br/api/feed/pt-br/PAINEL_INDICADORES/juros");
$content = $data->entry->content;
$selic = substr($content, 170, 5).'%';
$selic = str_replace('>','',$selic);
//Dolar
$content = '';
$data = simplexml_load_file("http://conteudo.bcb.gov.br/api/feed/pt-br/PAINEL_INDICADORES/cambio");
$content = $data->entry->content;
$ncompra = substr($content,27,6);
$compra  = substr($content,53,8);
$nvenda  = substr($content,98,5);
$venda   = substr($content,123,6);

$content = '';
$data = simplexml_load_file("http://conteudo.bcb.gov.br/api/feed/pt-br/PAINEL_INDICADORES/inflacao");
$content = $data->entry->content;
$inflac = substr($content,152,4);
...
if(isset($deldesp)){
  ...
  if ($sqlt[0] == '58' || $sqlt[0] == '57' || $sqlt[0] == '56' || $idusuario == '461' ||  $idusuario == '847' || $idusuario == '872' || $idusuario == '874'||  $idusuario == '214') { //and $idusuario != '847') {
...
if (isset($incdesp)){ 
  $vlrsel = valor_banco($val_despesa);
  $dup =  mysql_fetch_row(execsql("select grupo,descricao,val_despesa  from $mysql_despesa_table  where grupo = '".$grupo."' and descricao = '".$descricao."' and val_despesa = ".$vlrsel.""));
  if ($dup[0] >= 1){
	  	echo "<script>alert('Já existe despesa com esses valor, descrição e grupo. Registro será desprezado !');</script>";
  }else{
...
//	 $sql =execsql("insert into $mysql_despesa_table values(NULL, '$data',  '$grupo', '$descricao',".valor_banco($val_despesa).",'0.00','0.00','','','$obs_dir','$dvc','$centro')");
     $sql =execsql("insert into $mysql_despesa_table 
                    values(NULL, '$data',  '$grupo', '$descricao',".valor_banco($val_despesa).",'0.00','0.00','','','$obs_dir','$dvc','$centro','$dvc','9999999999')");
	 $sqlup =execsql("UPDATE $mysql_despesa_table SET cod_orig = codigo where data = $dt and cod_orig = '9999999999'");
...
if (isset($altdesp)){ 
  ...
  if ($sqlt[0] == '61' || $idusuario ==  '12') {
	 ...
	 if ($sqlt[1] == '874' || $sqlt[1] == '872' || $idusuario ==  '12') {
		 $dta  = substr($dtl,6,4).substr($dtl,3,2).substr($dtl,0,2);
//Altera venc $sql =execsql("UPDATE $mysql_despesa_table SET val_autoriz = '0.00', val_despesa = '".valor_banco($val_despesa)."', obs_dir = '".$obs_dir."', venc = '".$dvc."' , data = '".$ddt."' where codigo = $codigo");

	     $sql =execsql("UPDATE $mysql_despesa_table SET val_autoriz = '0.00', val_despesa = '".valor_banco($val_despesa)."', obs_dir = '".$obs_dir."', data = '".$ddt."' where codigo = $codigo");

	 }else{
//Altera venc $sql =execsql("UPDATE $mysql_despesa_table SET val_autoriz = '0.00', val_despesa = '".valor_banco($val_despesa)."', obs_dir = '".$obs_dir."', venc = '".$dvc."' where codigo = $codigo");

	     $sql =execsql("UPDATE $mysql_despesa_table SET val_autoriz = '0.00', val_despesa = '".valor_banco($val_despesa)."', obs_dir = '".$obs_dir."' where codigo = $codigo");
	 }
	...
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
					<TR> 
					<TD colspan=100% align=middle><B>Inclusão de Despesas 3</td>
						</TR>	
		</table>	</td>
			</tr>
		</table>	
			...
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=1>
	                    ...
	                    <tr>
							...
							<td width=50% class=back><select name=grupo >';createSelectgrupod2($idusuario);echo '</select></td>
						</tr>
						<tr>
...
if($act == "incbco"){
  $act = '';
  include "../common/data.php";
  echo "<form action=index.php name=incbco method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if($act == "inext"){
  $act = '';
  include "../common/data.php";
  echo "<form action=index.php name=inext method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if($act == "altext"){
  ...
  $act = '';
  include "../common/data.php";
  echo "<form action=index.php name=altext method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if($act == "altprv"){
  ...
  $act = '';
  include "../common/data.php";
  echo "<form action=index.php name=altprv method=post>";
	echo '
...
if($act == "inprv"){
  $act = '';
  include "../common/data.php";
  echo "<form action=index.php name=incpre method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=70% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if ($act == "trans") {
  ...
  $autor =  $row4[7];
  include "../common/data.php";
  echo "<form action=index.php name=autoriza method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if ($act == "deldesp") {
  ...
  $autor =  $row4[7];
  include "../common/data.php";
  echo "<form action=index.php name=deldesp method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
if ($act == "impdesp" && $local == '' && $hoje3 >= 20170414 ){
	...
	if (($idusuario == '872' || 
		 $idusuario == '12'  ||
                 $idusuario == '461' ||
		 $idusuario == '874' || 
		 $idusuario == '847') && $vlaut[0] <> 0) $vlaut[0] = '0';

    if ($vlaut[0] == 0 || $vlaut[0] == NULL ){
     	if ($idusuario == '214' || $idusuario == '12' || $idusuario == '461' || $idusuario == '872' || $idusuario == '847' || $idusuario == '874'||  $idusuario == '214') {
           $hoje2  = substr($dtl,6,4).substr($dtl,3,2).substr($dtl,0,2);
           $ontem = date('Ymd', strtotime('-1 days', strtotime($hoje2)));
           $now  = $dtl;
           $sql = "select codigo, grupo, val_autoriz, descricao,val_despesa,venc,centro,venc_orig, cod_orig from fluxocaixa.despesa where data = $ontem"; 
           $result = execsql($sql);
           ...
           while($row = mysql_fetch_row($result)){
	         ...
	         if ($pgt[0] < $row[4]){
	           ...
	           if ($dsp[0] > 0){
		 	      ...
		 	      execsql("insert into fluxocaixa.despesa values(NULL,'$hoje2','$row[1]','$row[3]','$dif','0.00','0.00','','','','$row[5]','$row[6]','$row[7]','$row[8]')");
		       }
	         }else{
...
if ($act == "pagar") {
  ...
  $autor =  $row4[7];
  include "../common/data.php";
  echo "<form action=index.php name=pagar method=post>";
	echo '
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
...
//if ($act2 == '1') {
   ...
   $dtlw = substr($dtl,6,4).substr($dtl,3,2).substr($dtl,0,2);
     ...
     <?  
//	   	 <input type="radio" name="indicador"> <b>indicadores</b>
       ...
       echo '<br><br>
        <table width="1000" border="1" align="center" cellpadding="0" cellspacing="0">
         <tr class="tdsubcabecalho3">
	       <td align="center" bgcolor="#EEDD82 "><font size="1" ><b> INDICADORES *B.Central - hoje </b></td>
	       <td align="center" bgcolor="#00FFFF ">SELIC:'.$selic.'</TD>
	       <td align="center" bgcolor="#FFBF00 ">US$ Cmp:'.$compra.'</TD>
	       <td align="center" bgcolor="#FFD700 ">US$ Vnd:'.$venda.'</TD>
	       <td align="center" bgcolor="#FFBF00 ">EU$ Cmp:'.$euro.'</TD>
	       <td align="center" bgcolor="#FFD700 ">IPCA(12):'.$inflac.'</TD>
	       <td align="center" bgcolor="#FFD700 ">CDI:'.$cdi.'</TD>
         </tr>
        </table>';
	}
?>
...
<table width="1000" border="1" align="center" cellpadding="0" cellspacing="0">
  <tr class="tdsubcabecalho3">
	<td align="center" bgcolor="#EEDD82 "><font size="3" ><b> E N T R A D A S</b></td><td></TD><td></TD><td></td><td></td><TD align="center" bgcolor="#FFFF00" font size="5" ><b>'.$hoje.'</b></TD>
		<td align="center"><a href="index.php?act=inext&dtl='.$hoje.'"><img src="images/plus.gif" title="Incluir" border="0"></a>

		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Bancos/Factorings</b>
		  	<a href="index.php?act=incbco&dtl='.$hoje.'"><img src="images/plus.gif" title="Incluir Banco" border="0"></a>
		  </td>
		  <td  align="center"><b>Saldo Inicial</b>';
        if ($local == ''){
		   echo'<a href="index.php?act=saldo&dtl='.$hoje.'"><img src="images/sald_ini.png" title="Atualizar Saldo" border="0"></a></td>';
		}
        echo '
...
if ($numero != 0 && $act == '') {
  while($row = mysql_fetch_row($result)){
		  ...
		  <td align="center">
		  <a href="index.php?act=altext&codigo=<?=$row[5]?>&banco=<?=$row[0]?>&dtl=<?=$hoje?>"><img src="images/alterar.gif" title="Alterar" border="0"></a>
		  <a href="index.php?act=delext&codigo=<?=$row[5]?>&dtl=<?=$hoje?>"><img src="images/deletar.gif" title="Deletar" border="0"></a>
...
?>
		...
		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Previsão de Entrada</b>
		 	<a href="index.php?act=incgrpp&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir Fomento" border="0"></a>
		  </td>
		  ...
		  <td  align="center"><b>Ação </b><a href="index.php?act=inprv&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir" border="0"></a></td> </tr>
<?
if ($local != '1006' && $local != '1008'){
...
if ($numero2 != 0 && $act == '') {
	  ...
	  while($row = mysql_fetch_row($result2)){	
		...
		<td align="center"><?=$bco[0]?></td>
		<td align="center">
	    <a href="index.php?act=altprv&codigo=<?=$row[5]?>&banco=<?=$row[1]?>&dtl=<?=$hoje?>"><img src="images/alterar.gif" title="Alterar" border="0"></a>
		<a href="index.php?act=delprev&codigo=<?=$row[5]?>&dtl=<?=$hoje?>"><img src="images/deletar.gif" title="Deletar" border="0"></a>
...
//	if ($dtl <= $hhatu){
     	...
     	?>
	     <a href="index.php?act=impdesp&dtl=<?=$hoje?>"><img src="images/sald_ini.png" title="Atualizar Despesas Até 10:00H" border="0">Autorizações poderão ser excluídas</a></td>
	    <?
//	}
	?>
	 </td><td></td><td></td><td></td><td></td>
     <td align="center"><a href="index.php?act=incdes&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir Despesa" border="0"></a></td>
		<?
	    if ($idusuario == '874' || $idusuario == '872' || $idusuario == '12'|| $idusuario == '847' || $idusuario == '214' || $idusuario == '48'  ){
        ?>
	    <td></td><td><a href="aucoletivo.php?dtl=<?=$hoje?>"><img src="images/autoriz.gif" title="Autoriz.Coletivo" border="0"><font size="3" ></a></td>
	    <?
	    }
	    if ($idusuario == '847' || $idusuario == '874' || $idusuario == '12'){
        ?>
	    <td><a href="pgcoletivo.php?dtl=<?=$hoje?>"><img src="images/pagar.png" title="Pagto.Coletivo" border="0"><font size="3" ></a></td>
	    <?
	    }
        if ($idusuario == '874' || $idusuario == '872' || $idusuario == '12'|| $idusuario == '847' ||  $idusuario == '214' ){
        ?>
		...
		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Grupo </b><a href="index.php?act=incgrpd&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir Grupo Despesas" border="0"></a></td>
    ...
    if ($local == '1006' || $local == '1008'){
       ...
       where data = $dtl  and centro = '$local' $selecgd order by cc.nome,gg.nome" );
//       where data = $dtl and venc = $dtl and centro = '$local' $selecgd order by cc.nome,gg.nome" );

	}else{
       $infbco = mysql_fetch_row(execsql("select sum(val_despesa-val_autoriz), sum(val_autoriz) 
	   from $mysql_despesa_table 
	   where data = $dtl group by data"));
//	   where data = $dtl and venc = $dtl group by data"));
       ...
       if ($usugrp[0] > 0) {
          ...
          inner join $mysql_grupo_table as gg ON gg.grupo = dd.grupo 
          inner join $mysql_categoria_table as cc ON gg.categoria = cc.categoria 
	      inner join $mysql_usu_categ_table as uu ON cc.categoria =  uu.categoria and uu.id = $idusuario
          where data = $dtl  $selecgd order by cc.nome,gg.nome" );
...
if ($numero3 != 0 && $act == '') {
	  ...
	  while($row = mysql_fetch_row($result3)){	
         ...
         if ($dtj >= $agora){
		  ...
		  <td align="center"><a href="index.php?act=trans&codigo=<?=$row[8]?>&disp=<?=($tsld - $infbco[1])?>&aautoriz=<?=$infbco[0]?>&dtl=<?=$hoje?>"><img src="images/cima.gif" title="Autorizar" border="0"></a>
		  <td align="center"><a href="pagar2.php?act=pagar&codigop=<?=$row[8]?>&dtl=<?=$hoje?>"><img src="images/final.gif" title="Pagar" border="0"></a>
		  <td align="center"><a href="index.php?act=altdes&codigo=<?=$row[8]?>&dtl=<?=$hoje?>"><img src="images/alterar.gif" title="Alterar" border="0"></a>
		                     <a href="index.php?act=deldesp&codigo=<?=$row[8]?>&dtl=<?=$hoje?>"><img src="images/deletar.gif" title="Deletar" border="0"></a>
...
?>
      ...
      <br><br>
	  <?
	  if ($idusuario != '874' && $idusuario != '872' && $idusuario != '12'){
	  ?>
      <tr class="tdsubcabecalho2">
...
////////////////// JAMES 11042019
	...
	<a href="index.php?act=incdes&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir" border="0"></a>
		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Grupo</b><a href="index.php?act=incgrpd&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir Grupo de Despesas" border="0"></a></td>
		  <td  align="center"><b>Subgrupo</b></td>
		  <td  align="center"><b>Descrição</b></td>
		  <td  align="center"><b>Valor Despesas</b></td>
		  <td  align="center"><b>Valor Autorizado</b></td>
...
//		  }
	  ...
	  <td align="center">T O T A L</td>
	  <td align="center"></td>
	  <td align="right"><?=number_format($tdesp2,'2',',','.')?> </td>
	  <td align="right"><?=number_format($taut2,'2',',','.')?> </td>
...
?>
	  ...
	  <td align="center"></td>
	  <td align="right"><?=number_format($tpgto2,'2',',','.')?> </td>
	  <td align="center"></td>
	  <td align="center"></td>
      <td align="center"></td></tr>
   	  <table width="500" border="1" align="center" cellpadding="0" cellspacing="0">
