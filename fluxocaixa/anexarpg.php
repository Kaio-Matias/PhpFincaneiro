...
if ($mostra == '1'){
  ...
  include "../common/data.php";
  echo "<form action=anexarpg.php name=incdes method=post>";
...
if ($grupo1 == '' || $grupo1 == '150') {
     ...
     if ($selgrupo == '') $selgrupo = 'p.grupo = '.$grupo1.' and ';
	}
...
<br><br><br><br>
		...
		<tr class="tdsubcabecalho1">
		  ...
		  <td  align="center"><b>Banco</b></td></tr>
<?
     if ($local <> ''){
         $result2 = execsql("select data,banco,descricao,val_prev ,val_real,codigo, val_negoc  from $mysql_previsao_table where data = $data1 and centro = $local");
     }else{
	     $result2 = execsql("select data,banco,descricao,val_prev ,val_real,codigo, val_negoc  from $mysql_previsao_table where data = $data1");
     }
  ...
  echo '
   <table width="1000" border="1" align="center" cellpadding="0" cellspacing="0">
    <br><br><br><br>
     ...
     <tr class="tdsubcabecalho3">
				<TD colspan=100% align=middle><B><font size="3">'.$dti.' a '.$dtf.'</font></td>
     </tr>	
...
if ($usugrp[0] > 0) {
    $result = execsql("select d.codigo, d.codigo2, p.descricao, d.banco, d.val_pg, DATE_FORMAT(d.dtpgto,'%d/%m/%Y') , p.grupo , g.nome, p.codigo, g.categoria from $mysql_pagto_det_table as d
                   inner join $mysql_despesa_table as p ON d.codigo = p.codigo
                   inner join $mysql_grupo_table as g ON g.grupo = p.grupo
	               inner join $mysql_usu_categ_table as uu ON g.categoria =  uu.categoria and uu.id = $idusuario
				   where $selgrupo d.dtpgto >= ".$data1." and d.dtpgto <= ".$data2." order by d.dtpgto, g.nome");
}else{
       $result = execsql("select d.codigo, d.codigo2, p.descricao, d.banco, d.val_pg, DATE_FORMAT(d.dtpgto,'%d/%m/%Y') , p.grupo , g.nome, p.codigo, g.categoria from $mysql_pagto_det_table as d
                   inner join $mysql_despesa_table as p ON d.codigo = p.codigo
                   inner join $mysql_grupo_table as g ON g.grupo = p.grupo
...
if ($local == ''){
       ...
       $result = execsql("select d.codigo, d.codigo2, p.descricao, d.banco, d.val_pg, DATE_FORMAT(d.dtpgto,'%d/%m/%Y') , p.grupo , g.nome, p.codigo,g.categoria from $mysql_pagto_det_table as d
                   inner join $mysql_despesa_table as p ON d.codigo = p.codigo
                   inner join $mysql_grupo_table as g ON g.grupo = p.grupo
				   where $selgrupo d.dtpgto >= ".$data1." and d.dtpgto <= ".$data2." and p.centro = $local order by d.dtpgto, g.nome");

}
*/
  while($row = mysql_fetch_row($result)){
          $bco    = mysql_fetch_row(execsql("select nome  from $mysql_banco_table where banco = $row[3]"));
          $nomeg  = mysql_fetch_row(execsql("select nome  from $mysql_grupo_table where grupo = $row[6]"));
          $nomec  = mysql_fetch_row(execsql("select nome  from $mysql_categoria_table where categoria = $row[9]"));
          $desc = $nomec[0].' - '.$nomeg[0];
          ...
          if ($row[5] <> $dtant){
			  if ( $tdia > 0){
	              ...
	              <tr class="tddetalhe1">
		           <td align="center"></td>
		           <td align="center"></td>
		           <td align="center"><?=number_format($tdia,'2',',','.')?></td>
				   <td align="center"></td>
				  </tr>
                  <?
			      $tdia = 0;
		  ...
		  $arq   = mysql_fetch_row(execsql("select nome, codigo,processo  from $mysql_anexo_table where processo = $row[8] AND tipo = 'D'"));
		  if ($arq[0] != "") {
			  echo '<tr bgcolor="#E0B2AA">';
		  }else{
			  echo '<tr class="$tdsubcabecalho2">';
	      }
		  ?>
		  <td align="left"><a href="anexo.php?codigop=<?=$row[0]?>&dtl=<?=$row[5]?>"><?=$desc?></a></td>
...
<br>
  <tr class="tdsubcabecalho2">
       ...
       <td></td>
       <td align="center"><?=number_format($tger,'2',',','.')?></td>
       <td></td>
  </tr>
  <?
