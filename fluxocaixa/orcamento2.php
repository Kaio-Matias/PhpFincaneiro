...
if ($mostra == '1'){
    $mostra = '';
  include "../common/data.php";
  echo "<form action=orcamento2.php name=incdes method=post>";
  ?>
...
if ($inext != '') {
  ...
  $agora = date('Ymd');
  if ($data1 < $agora) $data1 = date('Ymd');
  echo '
   <table width="1200" border="1" align="center" cellpadding="1" cellspacing="1">
    <br><br>
     <tr>';

//   if ($local == '') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - I - Geral</td>';
//   if ($local == '1006') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - I - Palmeira dos Índios</td>';
//   if ($local == '1008') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - I - Itapetinga</td>';
  echo '</tr>	
     <tr class="tdsubcabecalho3">
				<TD colspan=100% align=middle><B>'.$dti.' a '.$dtf.'</td>
     </tr>	';

  echo '<tr class="tddetalhe">
	    <td align="center"><font size="2">Despesa</font></td>
  ...
  if ($usugrp[0] > 0) {
      $result = execsql("select g.nome, sum(val_despesa), d.grupo, venc, centro from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   inner join  $mysql_categoria_table as c ON g.categoria = c.categoria
				   inner join  $mysql_usu_categ_table as u ON c.categoria = u.categoria and id = $idusuario
				   where $selgrupo data >= ".$data1." and data <= ".$data2." group by d.grupo");
  }else{
	        $result = execsql("select nome, sum(val_despesa), d.grupo, venc, centro from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." group by d.grupo");
  ...
  while($row = mysql_fetch_row($result)){
	echo '
		  <tr id="'.$row[2].'"> 
			<td nowrap  align="left"><font size="1"><a href="#" onclick="toggleRows(this)" class="folder"><B>'.$row[0].'</b></a></font></td>
			<td nowrap  align="right"><font size="1"><B>'.number_format($row[1],'2',',','.').'</B></font></td>
			<td nowrap  align="center"><font size="1">-</font></td>
		  </tr>';

     $result2 = execsql("select descricao, val_despesa, DATE_FORMAT(venc,'%d/%m/%Y'), d.grupo from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." and d.grupo = ".$row[2]." and centro = $row[4] order by venc");

     while($row2 = mysql_fetch_row($result2)){
			if ($i % 2) { $cor = $cores['tddetalhe1'];} else { $cor = $cores['tdfundo']; }	if ($i % 2) { $cor2 = $cores['tddetalhe2'];} else { $cor2 = ""; }
			if ($row2[0] == '') $row2[0] = 'Não informado';
			echo '
			  <tr id="'.$row2[3].'"> 
     			<td nowrap  align="left"><font size="0">'.$row2[0].'</font></td>
	    		<td nowrap  align="right"><font size="0">'.number_format($row2[1],'2',',','.').'</font></td>
		    	<td nowrap  align="center"><font size="0">'.$row2[2].'</font></td>
		      </tr>';
 		      $i++;
			  $ttot += $row2[1];
	}
  	...
  	echo '
			  <tr id="'.$row2[3].'"> 
     			<td nowrap  align="left"><font size="0">TOTAL</font></td>
	    		<td nowrap  align="right"><font size="0"><B>'.number_format($ttot,'2',',','.').'</B></font></td>
		    	<td nowrap  align="center"><font size="0">*</font></td>
		      </tr>';
