...
if ($mostra == '1'){
    $mostra = '';
  include "../common/data.php";
  echo "<form action=orcamento3.php name=incdes method=post>";
  ?>
...
if ($inext != '') {
  ...
  echo '
   <table width="1200" border="1" align="center" cellpadding="1" cellspacing="1">
    <br><br>
     <tr>
				<TD colspan=100% align=middle><B>Previsão de Pagamento - I</td>
     </tr>	
  ...
  $result = execsql("select nome, sum(val_despesa), d.grupo, venc from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." order by venc, d.grupo");
  ...
  while($row = mysql_fetch_row($result)){
	echo '
     ...
     $result2 = execsql("select descricao, val_despesa, venc, d.grupo from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo venc = ".$row[3]." and d.grupo = ".$row[2]."");
