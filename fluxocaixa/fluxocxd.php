...
if ($avancar == 'SIM')
    ...
    <table width="350" border="0" align="center" cellpadding="2" cellspacing="1">
     ...
     <tr class="tddetalhe1"> 
      <td align="center"><a href="fluxocxd.php">Lançar Novo Fluxo Caixa</a></center></td>
     </tr>
     <tr class="tddetalhe1"> 
	...
	<?
	execsql("DELETE FROM $mysql_fluxocxd_table WHERE centro='".$centro."' and datprv = ".$datprv."");
	...
	while($row = mysql_fetch_row($result)){
	    ...
	    $sql = execsql("insert into $mysql_fluxocxd_table values (".$centro.",".$row[1].",".$row[0].",'".$datprv."',".$mes.",".$ano.",".valor_banco($$preco).",".valor_banco($$preco2).",".valor_banco($$preco3).")");
	}
		exit;
	}

?>
<form name="form1" method="post" action="fluxocxd.php">
  <p align="center"><font size = "3">Digite previsão diária fluxo de caixa - Saídas.</font></p>
...
<br>
   <table width="500" border="0" align="center" cellpadding="0" cellspacing="1">
	...
	while($row = mysql_fetch_row($result)){
		$dtprv = $ano.$mes.$dia;
    	$prev = mysql_fetch_row(execsql("select valor,valorr,valort from $mysql_fluxocxd_table WHERE grupo = '".$row[0]."' and datprv='".$dtprv."' and centro='".$centro."'"));
	...
	?>
          <tr> 
            <td colspan="2" align="left"><br><a href="fluxocxd.php"><img src="images/btvoltar.gif"  border="0"></a></td>
...
<input name="datprv"  type="hidden" size="6" value="<?=$ano.$mes.$dia;?>">
...
<?
	$i = 1;
	$sql = "select * from $mysql_fluxocxd_table WHERE datprv = concat($ano.$mes.$dia)  and centro='".$centro."' order by centro, categoria, grupo";
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)){
...
<table width="580" border="0" align="center">
  <tr> 
    <td align="center" class="tdcabecalho"><b>Previsão Fluxo de Caixa Diário - Saídas</b></td>
  </tr>
  <tr>
