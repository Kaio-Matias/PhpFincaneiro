...
if ($inext != '') {
   ...
   if ($local == '') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - Geral</td>';
   if ($local == '1006') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - Palmeira dos Índios</td>';
   if ($local == '1008') echo '<TD colspan=100% align=middle><B>Previsão de Pagamento - Itapetinga</td>';
   echo ' </tr>	
        <tr class="tdsubcabecalho3">
...
///
  ...
  if ($usugrp[0] > 0) {
   $result = execsql("select g.nome, sum(val_despesa) $select from $mysql_despesa_table as d
...
/*
  if ($local == ''){
   ...
   $result = execsql("select nome, sum(val_despesa) $select from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." and centro = $local");
  }
  */
...
///
  ...
  if ($usugrp[0] > 0) {
   ...
   $result = execsql("select g.nome, sum(val_despesa) $select from $mysql_despesa_table as d
				   ...
				   inner join  $mysql_categoria_table as c ON g.categoria = c.categoria
				   inner join  $mysql_usu_categ_table as u ON c.categoria = u.categoria and id = $idusuario
				   where $selgrupo data >= ".$data1." and data <= ".$data2." group by g.nome order by nome");
  }else{
   $result = execsql("select g.nome, sum(val_despesa) $select from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." group by g.nome order by nome");
