...
**	file:	RelpagtoC.php
**
**		Relação de Pagamento Consolidado	
**
***************************************************************************************************
**
**	author:	James Reig
...
if ($mostra == '1'){
    $mostra = '';
  include "../common/data.php";
  echo "<form action=relpagtoc.php name=incdes method=post>";
  ?>
...
$wdata2 = $data2;
  ...
  if ($local == '')     echo '<TD colspan=100% align=center><B><font size="5">Consolidado - Grupo Valedourado</font></td>';
  if ($local == '1006') echo '<TD colspan=100% align=center><B><font size="5">Consolidado - Palmeira dos Índios</font></td>';
  if ($local == '1008') echo '<TD colspan=100% align=center><B><font size="5">Consolidado - Itapetinga</font></td>';
  $fab = '';
  if ($local == ''){
	  $fab = '';
  }else{
  ...
  echo '</tr>	
     <tr class="tdsubcabecalho3">
				<TD colspan=100% align=center><B><font size="4">Pagamentos Consolidados - '.$dti.' A '.$dtf.'</font></td>
     </tr>	';
...
//////////////////////////////
  ...
  while($row = mysql_fetch_row($result)){
	if ($row[1] == 3){
	   $vlpago      = execsql("select sum(val_pg), g.grupo, g.nome from $mysql_despesa_table as d
	  			                                   inner join  $mysql_pagto_det_table as p ON d.codigo = p.codigo
                                                   inner join  $mysql_grupo_table as g ON d.grupo = g.grupo
				   where $fab dtpgto >= ".$wdata1." and dtpgto <= ".$wdata2." and g.categoria = ".$row[1]." group by g.grupo");

	}else{
	   $vlpago      = execsql("select sum(val_pg) from $mysql_despesa_table as d
	  			                                   inner join  $mysql_pagto_det_table as p ON d.codigo = p.codigo
                                                   inner join  $mysql_grupo_table as g ON d.grupo = g.grupo
				   where $fab dtpgto >= ".$wdata1." and dtpgto <= ".$wdata2." and g.categoria = ".$row[1]." group by g.categoria");
	}
	  while($row2 = mysql_fetch_row($vlpago)){
	    $vlpp = $row2[0];
        $ttotp += $vlpp;
	    echo '
		  <tr class="tdsubcabecalho1"> 
			<td nowrap  align="left"><font size="1">'.$row[0].'&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp '.$row2[2].'</font></td>
    ...
    if ($fab == NULL){
    $sdfinal = $saldoini[0] + $ttotpe - $ttotp;
    echo '
		  <tr class="tdsubcabecalho1"> 
			<td nowrap  align="left"><font size="1">SALDO FINAL</font></td>
			<td nowrap  align="right"><font size="1"><B>'.number_format($sdfinal,'2',',','.').'</B></font></td>
		  </tr>';
    }
}
?>
