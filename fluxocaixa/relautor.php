...
if ($mostra == '1'){
  ...
  include "../common/data.php";
  echo "<form action=relautor.php name=incdes method=post>";
...
$numero2 = mysql_num_rows($result2);
	  ...
	  while($row = mysql_fetch_row($result2)){	
        ...
        if ($local != '1006' && $local != '1008'){
            $ttaut = mysql_fetch_row(execsql("select sum(val_autoriz) from $mysql_despesa_table  where data = $data1"));
	    }else{
            $ttaut = mysql_fetch_row(execsql("select sum(val_autoriz) from $mysql_despesa_table  where data = $data1 and centro = $local"));
        }
		?>
		<tr></tr><br><br>
...
///////////////////
  echo '
   ...
   if ($local == '') echo '<TD colspan=100% align=middle><B>Relação de Pagamento Autorizados - Geral</td>';
   if ($local == '1006') echo '<TD colspan=100% align=middle><B>Relação de Pagamento Autorizados - Palmeira dos Índios</td>';
   if ($local == '1008') echo '<TD colspan=100% align=middle><B>Relação de Pagamento Autorizados - Itapetinga</td>';
  ...
  if ($local == ''){
      $result = execsql("select nome, descricao, val_autoriz, autoriza from $mysql_despesa_table as p 
                   inner join $mysql_grupo_table as g ON g.grupo = p.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." and val_autoriz > 0 order by data, g.nome");
  }else{
      $result = execsql("select nome, descricao, val_autoriz, autoriza from $mysql_despesa_table as p 
                   inner join $mysql_grupo_table as g ON g.grupo = p.grupo
				   where $selgrupo data >= ".$data1." and data <= ".$data2." and val_autoriz > 0 and p.centro = $local order by data, g.nome");

  }		

  while($row = mysql_fetch_row($result)){
          $aut    = mysql_fetch_row(execsql("select login  from UMDnew.usuarios where id = $row[3]"));
          if ($row[5] <> $dtant){
			  if ( $tdia > 0){
	        	  ?>
	              <tr class="tddetalhe1">
