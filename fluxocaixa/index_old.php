...
if (isset($incext)){ 
  ...
  if ($sqlt[0] == '58' || $sqlt[0] == '57' || $sqlt[0] == '56') {
	 ...
	 $sql =execsql("insert into $mysql_extrato_table values (NULL, '$data',  '$banco', '',".valor_banco($sald_ini).",".valor_banco($entrada).",'0.00','0.00')");
...
if (isset($incdesp)){ 
  ...
  if ($sqlt[0] == '58' || $sqlt[0] == '56' || $sqlt[0] == '57' || $sqlt[0] == '59') {
	 ...
	 $sql =execsql("insert into $mysql_despesa_table values(NULL, '$data',  '$grupo', '$descricao',".valor_banco($val_despesa).",'0.00','0.00','','','$obs_dir','$dvc')");
...
if (isset($altdesp)){ 
  ...
  if ($sqlt[0] == '61') {
	 ...
	 if ($sqlt[1] == '872') {
	     ...
	     $sql =execsql("UPDATE $mysql_despesa_table SET val_autoriz = '0.00', val_despesa = '".valor_banco($val_despesa)."', obs_dir = '".$obs_dir."', venc = '".$dvc."' , data = '".$dta."' where codigo = $codigo");
...
if (isset($incgdesp)){ 
     $dt = date('Y-m-d');
	 $sql =execsql("insert into $mysql_grupo_table values(NULL,'$categoria', '$descricao')");
...
if (isset($incprev)){ 
  ...
  if ($sqlt[0] == '58' || $sqlt[0] == '59' || $sqlt[0] == '57' || $sqlt[0] == '56') {
     $dt = date('Y-m-d');
     $data = substr($dtl,6,4).substr($dtl,3,2).substr($dtl,0,2);
	 $sql =execsql("insert into $mysql_previsao_table values (NULL,'$data','$banco','$descricao' ,".valor_banco($val_prev).",".valor_banco($val_real).",'$selecionado[0]','0.00','0.00')");
		...
		<TABLE class=border bgcolor=F5F5F5 cellSpacing=0 cellPadding=0 width=100% align=center border=1>
			...
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=1>
	                  <tr>
							<td width=10% class=tdsubcabecalho1 align=right>Subgrupo:</td>
			...
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=1>
	                  <tr>
					        ...
					        <td width="20%"><input size="10" name="dtl" value='.$dtl.'></td>
  ...
  echo "<form action=index.php name=incbco method=post>";
	...
	<TABLE class=border cellSpacing=0 cellPadding=0 width=100% align=center border=1>			<TR> 
			<TD> 
				<TABLE cellSpacing=1 cellPadding=2 width=80% border=0>
					<TR> 
					<TD colspan=100% align=middle><B>Banco/Agência</td>
						</TR>	
		</table>	</td>
			</tr>
		</table>	
				...
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=1>
	                  <tr>
						...
						<tr>
					        ...
					        <td width="20%"><input size="15" name="val_negoc" value='.number_format($prev[7],'2',',','.').'></td>
					        <td align="right" class="tdsubcabecalho1" width="30%">Valor Recompra:</td>
					        <td width="20%"><input size="15" name="recompra" value='.number_format($prev[8],'2',',','.').'></td>
						</tr>
					    <tr>
				        	<td align="right" class="tdsubcabecalho1" width="30%">Valor Previsto:</td>
					        <td width="20%"><input size="15" name="val_prev" value='.number_format($prev[4],'2',',','.').'></td>
					        <td align="right" class="tdsubcabecalho1" width="30%">Valor Real:</td>
					        <td width="20%"><input size="15" name="val_real" value='.number_format($prev[5],'2',',','.').'></td>
						</tr>';
...
if ($act == "saldo") {
    ...
    $sql = "select codigo, data,banco,sald_ini,entrada from fluxocaixa.extrato where data = $ontem";
    $result = execsql($sql);
    while($row = mysql_fetch_row($result)){
       $prv = mysql_fetch_row(execsql("select sum(val_real) from fluxocaixa.previsao where banco = $row[2] and data = $ontem  group by banco"));
       $pgt = mysql_fetch_row(execsql("select sum(val_pg) from fluxocaixa.pagto_det where banco = $row[2] and dtpgto = $ontem   group by banco"));
       $sald_ini = $row[3] + $row[4] + $prv[0] - $pgt[0];
       $ext = mysql_fetch_row(execsql("select banco from fluxocaixa.extrato where banco = $row[2] and data = $hoje2"));
       if ($ext[0] > 0) {
         execsql("update fluxocaixa.extrato set sald_ini = ".$sald_ini." where banco = $row[2] and data = $hoje2");
       }else{
         execsql("insert into fluxocaixa.extrato values(NULL, '$hoje2','$row[2]','','$sald_ini','0.00','0.00','0.00')");
       }
    }
}
...
if ($act == "impdesp"){
	...
	if (($idusuario == '872' || $idusuario == '15' || $idusuario == '874' || $idusuario == '873') && $vlaut[0] <> 0) $vlaut[0] = '0';

    if ($vlaut[0] == 0 || $vlaut[0] == NULL ){
     	if ($idusuario == '874' || $idusuario == '15' || $idusuario == '872' || $idusuario == '873') {
           $hoje2  = substr($dtl,6,4).substr($dtl,3,2).substr($dtl,0,2);
           $ontem = date('Ymd', strtotime('-1 days', strtotime($hoje2)));
           $now  = $dtl;
           $sql = "select codigo, grupo, val_autoriz, descricao,val_despesa,venc from fluxocaixa.despesa where data = $ontem"; 
           $result = execsql($sql);
           ...
           while($row = mysql_fetch_row($result)){
	         ...
	         if ($pgt[0] < $row[4]){
	           ...
	           if ($dsp[0] > 0){
		 	      ...
		 	      execsql("insert into fluxocaixa.despesa values(NULL, '$hoje2','$row[1]','$row[3]','$dif','0.00','0.00','','','','$row[5]')");
		       }
	         }else{
...
//if ($act2 == '1') {
   ...
   <?
   echo ' <tr>
	  <td class=tdsubcabecalho1 width=20% align=right>Data Seleção:</td>';
   ?>
     <td width="25%"><input type="text" name="dtl" size="11" maxlength="11" onFocus="javascript:vDateType='3'" onKeyUp="DateFormat(this,this.value,event,false,'3')" onBlur="DateFormat(this,this.value,event,true,'3')" value="<?=$dtl?>"> <INPUT onclick="return showCalendar('dtl', 'dd/mm/y');" type=reset value=" ... "></td></tr>
   <?
...
<table width="1000" border="2" align="center" cellpadding="0" cellspacing="0">
  <tr class="tdsubcabecalho1">
       <td align="center"><font size="2" ><b>Caixa Diário</b></td>
	  </tr>
</TABLE>
<?
	...
	<td align="center" bgcolor="#EEDD82 "><font size="3" ><b> E N T R A D A S</b></td><td></TD><td></TD><td></td><td></td><TD align="center" bgcolor="#FFFF00" font size="5" ><b>'.$hoje.'</b></TD>
		...
		<tr class="tdsubcabecalho1">
		  ...
		  <td  align="center"><b>Saldo Inicial</b>
		  	<a href="index.php?act=saldo&dtl='.$hoje.'"><img src="images/sald_ini.png" title="Atualizar Saldo" border="0"></a></td>
...
?>
  	  ...
  	  <table width="1000" border="2" align="center" cellpadding="0" cellspacing="0">
		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Previsão de Entrada</b></td>
...
$result2 = execsql("select data,banco,descricao,val_prev ,val_real,codigo  from $mysql_previsao_table where data = $dtl");
...
// Despesas
  ...
  if ($sqlt[0] == '61') {
	...
	<a href="index.php?act=incdes&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir" border="0"></a>
		<tr class="tdsubcabecalho1">
		  <td  align="center"><b>Grupo Despesas</b><a href="index.php?act=incgrpd&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir Grupo de Despesas" border="0"></a></td>
...
if ($numero3 != 0 && $act == '') {
	  ...
	  while($row = mysql_fetch_row($result3)){	
          $grupo = mysql_fetch_row(execsql("select nome  from $mysql_grupo_table where grupo = $row[0]"));
...
//////////////////
	?>
   	...
   	<table width="1200" border="2" align="center" cellpadding="0" cellspacing="0">
	<td align="center" bgcolor="#20B2AA"><font size="3" ><b> V E N C I D A S</b></td><td align="center">
	<a href="index.php?act=impdesp&dtl=<?=$hoje?>"><img src="images/sald_ini.png" title="Atualizar Despesas Até 10:00H" border="0">Autorizações poderão ser excluídas</a></td>
	</td><td></td><td></td><td></td><td></td><td></td><td></td></td><td align="center">
	<a href="index.php?act=incdes&dtl=<?=$hoje?>"><img src="images/plus.gif" title="Incluir" border="0"></a>
		<tr class="tdsubcabecalho1">
