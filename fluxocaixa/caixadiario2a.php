...
///
   ...
   $result = execsql("select g.nome, sum(val_pg) $select from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   inner join  $mysql_pagto_det_table p ON p.codigo = d.codigo
				   where dtpgto >= ".$data1." and dtpgto <= ".$data2." group by g.nome order by nome");
...
///
   ...
   $venc = mysql_fetch_row(execsql("select sum(val_despesa) from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where data = ".$ontem." and venc < ".$data1."")) ;
   $pgto = mysql_fetch_row(execsql("select sum(val_pg ) from $mysql_despesa_table as d
                   ...
                   where dtpgto = ".$ontem."")) ;
echo '<br>'.'Vencida = '.$venc[0].' Pago = '.$pgto[0].'<br>';
  ...
  ?>
     ...
     <td align="center">TOTAL</td>
     <td align="right" bgcolor="#FFAA1"><?=number_format(($row[1]),'2',',','.')?></td>
     <td align="right" bgcolor="#FFAA2"><?=number_format(($pgto[0]),'2',',','.')?></td>
...
///
   $resultd = execsql("select g.nome $selectd2 from $mysql_despesa_table as d
                   inner join  $mysql_grupo_table as g ON g.grupo = d.grupo
				   where (data >= ".$data1." and data <= ".$data2.") and 
				         (venc >= ".$data1." and venc <= ".$data2.") group by g.nome order by nome");
