...
$versaogvendas = "2.0";							// Versão do Gestão de Vendas
$gvendas_name = "Gestão de Vendas - Valedourado";
...
function createSelectBase($form = 'n')
{
	...
	$result = execsql("select d.client, d.nome from  $mysql_grpestrutorg_table a, $mysql_usrgrpusuarios_table b,$mysql_estrutorg_table c, $mysql_base_table d where b.id = '".$user_info['id']."' and b.idgrupousuario = a.idgrupousuario and c.cod_aplicacao = 'GESTVENDAS' and a.idestrutorg = c.idestrutorg and c.idestrutorg = '4' and a.valor = d.client group by d.client");
...
function createSelectCentro2($centro)
{
	global $mysql_centros_table, $centro,$info;
  	$sql = "select centro local, nome from gvendas.centros order by centro asc";
	$result = execsql($sql);
...
function MostrarCentro2($centro)
{
	global $mysql_centros_table;

	$sql = "select centro, nome from $mysql_centros_table where centro = '".$centro."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
...
/***********************************************************************************************************
**	function MostrarFilial($codfilial):
**		Mostra a Filial sem o código da mesma
************************************************************************************************************/

function MostrarFilial2($codfilial)
{
	global $mysql_filiais_table;
...
/***********************************************************************************************************
**	function Mostrartipofatura($codtipofatura):
**		Mostra o tipo de fatura com o código da mesma
************************************************************************************************************/

function Mostrartpfatura($codtipofatura)
{
	global $mysql_tipofatura_table;

	$sql = "select codtipofatura, nome from $mysql_tipofatura_table where codtipofatura = '".$codtipofatura."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".ucfirst(strtolower($row[1]));
}
...
function PositivacaoVendedor($codvendedor,$data)
{
	...
	$result = execsql("
	select codcliente, codproduto, datafatura, sum(valorbruto+valordesconto+valoradicional) from $mysql_vendas_table 
...
/***********************************************************************************************************
**	function createSelectPromotor():
**		Cria select das filiais de acordo com a permissão do usuário
**		Obs.: $cookie_name -> login do usuário.
************************************************************************************************************/

function createSelectPromotor($codfilial) {
	global $mysql_promotor_table;
	$filial = PermissaoFinal("codfilial","in");
 //       $result=execsql("SELECT promotor, nome FROM $mysql_promotor_table where codfilial $filial GROUP BY promotor order by nome ");
        $result=execsql("SELECT promotor, nome FROM $mysql_promotor_table where codfilial ='".$codfilial."' GROUP BY promotor order by nome ");
...
function MostrarPromotor($codfilial) { 
	global $mysql_promotor_table;
    $sql = "SELECT promotor, nome, codfilial FROM $mysql_promotor_table where promotor='".$codfilial."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
function MostrarFiliais($codfilial) { 
	global $mysql_filiais_table;
    $sql = "SELECT * FROM $mysql_filiais_table order by nome";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
...
function createSelectCliente2($codfilial) {
	global $mysql_clientes_table;

    $result=execsql("SELECT codcliente, nome FROM $mysql_clientes_table where codfilial = '$codfilial' order by nome ");
	$resultado = "<select name='selectcliente' style='width: 350px;'>";
...
function createSelectStatusC($nomeselect,$statusc = '')
{
	global $mysql_statusc_table;

	$sql = "select statuscarga, nome from $mysql_statusc_table order by nome";
	$result = execsql($sql);
	$return = "<select name='$nomeselect' style='width: 150px;'>";

	while($row = mysql_fetch_array($result)){
		if ($statusc == $row[0]) { $select = "selected"; } else { $select = ""; };
		$return .= "<option value=\"$row[0]\" $select>".$row[1];
	}
	$return .= "</select>";
	return $return;
...
/***********************************************************************************************************
**	function NomeCliente($codccliente):
**		Mostra a Filial com o código da mesma
************************************************************************************************************/

function NomeCliente($codcliente)
{
	global  $mysql_clientes_table;
