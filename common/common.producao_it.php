...
$producao_name = "Controle de Produção - Itapetinga";
...
function UltimaAtualizacao($carga)
{
	global $mysql_atualizacaoi_table;

	$sql = "select date_format(data, '%d/%m/%Y %H:%i:%s') from $mysql_atualizacaoi_table where sistema = '$carga' order by data desc";
	$result = execsql($sql);
...
function getQtdePlts($tunel) {
	global $mysql_estoquei_table;
	$r = mysql_fetch_row(execsql("select count(*) from $mysql_estoquei_table where tunel='$tunel' and desceu='N' and excluido='N'"));
	return $r[0];
}
...
{
	global $mysql_produtos_table, $mysql_produtos_quanti_table;

	$res = execsql("select codproduto from $mysql_produtos_quanti_table");
...
{
	global $mysql_produto_enderecoi_table, $mysql_estoquei_table;
	
	$result = execsql("select codendereco, count(*) from $mysql_produto_enderecoi_table left join $mysql_estoquei_table on codendereco=tunel where codproduto='$codigoproduto' group by codendereco having count(*)<25");
...
{
	global $mysql_enderecosi_table, $mysql_produto_enderecoi_table;

	$res = execsql("select codendereco from $mysql_produto_enderecoi_table");
	while($r = mysql_fetch_row($res))
		$t .= "'$r[0]',";
	if(strlen($t)>0)
	...
	$result = execsql("select codendereco from $mysql_enderecosi_table $where order by codendereco");
	while($row = mysql_fetch_row($result)) {
		echo "<option value=\"$row[0]\">".$row[0]."</option>";
	}
	echo "</select>";
...
{
	global $mysql_basei_table, $cookie_name,	$mysql_grpestrutorgi_table , $mysql_usrgrpusuariosi_table ,$mysql_estrutorgi_table, $mysql_basei_table;
	$user_info = getUserInfo(getUserID($cookie_name));

	$result = execsql("select d.client, d.nome from  $mysql_grpestrutorgi_table a, $mysql_usrgrpusuariosi_table b,$mysql_estrutorgi_table c, $mysql_basei_table d where b.id = '".$user_info['id']."' and b.idgrupousuario = a.idgrupousuario and c.cod_aplicacao = 'PRODUCAO' and a.idestrutorg = c.idestrutorg and c.idestrutorg = '4' and a.valor = d.client group by d.client");
...
function PermissaoVendedor($codvendedor)
{
	global $mysql_vendedoresi_table;
	$filiais = PermissaoFinal("codfilial","in");
	$sql = "select a.nome from $mysql_vendedoresi_table a where codfilial ".$filiais." and codvendedor = $codvendedor";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
...
function EmviarMetas($codvendedor,$mes,$ano)
{
	global $mysql_metavendedori_table, $mysql_vendasi_table, $bonificacao;
...
if (substr($codvendedor,1,2) >= "80") {
	$result = execsql("select codproduto from $mysql_metavendedori_table where ((codvendedor = '".$codvendedor."' and codsupervisor != '".$codvendedor."') or (codsupervisor = '".$codvendedor."' and codvendedor != '".$codvendedor."' ) or (codsupervisor = '".$codvendedor."' and codvendedor = '".$codvendedor."' )) and mes = '".$mes."' and ano = '".$ano."' order by codproduto");    // query que ira criar a matriz com todos os produtos que tenham ocorrencia
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
	$result = execsql("select distinct codproduto from $mysql_vendasi_table where codvendedor = '".$codvendedor."' and client = '150' and datafatura like '".$ano.$mes."%' and codtipofatura $bonificacao order by codproduto");   // query que ira criar a matriz com todos os produtos que tenham ocorrencia
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
	if ($vendas != '') {			// $vendas e a matriz que contem todos os codproduto com ocorrencia.
		...
		while (list ($chave, $valor) = each ($vendas)) {
			//				Seleciona as metas
			$result = execsql("select sum(quantidade), avg(precomedio) from $mysql_metavendedori_table where ((codvendedor = '".$codvendedor."' and codsupervisor != '".$codvendedor."') or (codsupervisor = '".$codvendedor."' and codvendedor != '".$codvendedor."' ) or (codsupervisor = '".$codvendedor."' and codvendedor = '".$codvendedor."' )) and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'");
			$row = mysql_fetch_row($result);
			...
			$qntvendas = 0;
			$vlrvendas = 0;
			$resultado = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_vendasi_table where client = '150' and  codvendedor = '".$codvendedor."' and datafatura like '".$ano.$mes."%' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
			$vendedor = mysql_fetch_array($resultado);
			$qntvendas = $vendedor[0] + $qntvendas;
			$vlrvendas = $vendedor[1] + $vlrvendas;

			$result = execsql("select codgrpcliente, codfilial, codvendedor from $mysql_metavendedori_table where codsupervisor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'  group by codgrpcliente, codfilial, codvendedor");
			while($row = mysql_fetch_array($result)){
				$result3 = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_vendasi_table where client = '150' and  codgrpcliente = '".$row[0]."' and codfilial = '".$row[1]."' and codvendedor = '".$row[2]."' and datafatura like '".$ano.$mes."%' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
...
";
	...
	$result = execsql("select distinct codproduto from $mysql_vendasi_table where client = '150' and  codvendedor = '".$codvendedor."' and datafatura like '".$ano.$mes."%' order by codproduto  and codtipofatura $bonificacao");
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
	$result = execsql("select codproduto from $mysql_metavendedori_table where codvendedor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."'");
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
...
if ($vendas != '') {
	...
	while (list ($chave, $valor) = each ($vendas)) {
		//				Seleciona as metas
		$result = execsql("select sum(quantidade), avg(precomedio) from $mysql_metavendedori_table where codvendedor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'");
		$row = mysql_fetch_row($result);
		$conteudo .= str_pad($valor, 18);
		$conteudo .= str_pad(str_replace('.','',number_format($row[0],'2','','')),9,'0',STR_PAD_LEFT);
		$conteudo .= str_pad(str_replace('.','',number_format($row[0]*$row[1],'2','','')),8,' ',STR_PAD_LEFT);
		//				Seleciona o realizado
		$result = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_vendasi_table where client = '150' and codvendedor = '".$codvendedor."' and datafatura like '".$ano.$mes."%' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
		$row = mysql_fetch_row($result);
...
function createSelectVendedores($filial,$vendedor = '')
{
	...
	$sql = "select codvendedor, nome from $mysql_vendedoresi_table where codfilial = '$filial' and nome != '' order by codvendedor";
	$result = execsql($sql);
...
function createSelectSupervisor($filial,$vendedor = '',$nome = 'selectsupervisor')
{
	...
	$result = execsql("select codvendedor, nome from $mysql_vendedoresi_table where codfilial = '$filial' and nome != '' and codvendedor >= '".SUBSTR($filial,-1,1)."80' order by codvendedor");
...
$result = execsql("select data from $mysql_feriadosi_table");
while($row = mysql_fetch_array($result)){
	$feria[$row[0]] = "ok";
}
...
function MostrarFilial($codfilial)
{
	global $mysql_filiaisi_table;

	$sql = "select codfilial, nome from $mysql_filiaisi_table where codfilial = '".$codfilial."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
...
{
	global $mysql_nomefantasiai_table;

	$sql = "select codfilial, nomefantasia from $mysql_nomefantasiai_table where codfilial = '".$codfilial."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[1];
}
...
function MostrarCX($codvendedor)
{
	global  $mysql_vendedorcxi_table;

	$sql = "select cx from $mysql_vendedorcxi_table where codvendedor = '".$codvendedor."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0];
}
...
function MostrarConcorrente($codconcorrente)
{
	global  $mysql_concorrentesi_table;

	$sql = "select nome from $mysql_concorrentesi_table where codconcorrente = '".$codconcorrente."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0];
}
...
{
		global  $mysql_clientesi_table;

	$sql = "select codcliente, nome from $mysql_clientesi_table where nome like '%".$nome."%' order by nome asc";
	$result = execsql($sql);
...
{
	global $mysql_grupoprodutoi_table;

	$sql = "select nome from $mysql_grupoprodutoi_table where codgrpproduto = '".$codgrpproduto."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0];
}
...
function MostrarVendedor($codvendedor)
{
	...
	$sql = "select codvendedor, nome from $mysql_vendedoresi_table where codvendedor = '".$codvendedor."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
...
function MostrarCanal($codcanal)
{
	global $mysql_grupoclientei_table;

	$sql = "select codgrpcliente, nome from $mysql_grupoclientei_table where codgrpcliente = '".$codcanal."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".ucfirst(strtolower($row[1]));
}
...
{
	global $mysql_basei_table;

	$sql = "select client, nome from $mysql_basei_table where client = '".$codbase."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".ucfirst(strtolower($row[1]));
}
...
function MostrarMR($mr)
{
	global $mysql_motivorecusai_table;

	$sql = "select codmotivorecusa, nome from $mysql_motivorecusai_table where codmotivorecusa = '".$mr."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".ucfirst(strtolower($row[1]));
}
...
function GerarSaldo($filial, $mes, $ano, $codproduto)
{
	...
	global $mysql_produtos_table, $mysql_metafiliali_table, $mysql_metavendedori_table;

	$sql = "select a.codproduto, b.quantidade from $mysql_produtos_table a, $mysql_metafiliali_table b where a.codfilial = b.codfilial and a.codfilial = '$filial' and b.codproduto = a.codproduto and b.codproduto = '$codproduto' and b.mes = '$mes' and b.ano = '$ano' order by a.nome";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);

	$result2 = execsql("select sum(quantidade) from $mysql_metavendedori_table where codfilial = '$filial' and codproduto = '$codproduto' and mes = '$mes' and ano = '$ano' group by codfilial, codproduto, mes, ano");
...
function ListarFiliaisCanaisVendedores($mes,$ano)
{
global $mysql_metavendedori_table, $mysql_vendedorcxi_table;

$qntcanal = '0';
$qntsupervisor = '0';
$qntvendedor = '0';

$sql = "select a.codfilial from $mysql_metavendedori_table a where a.ano = '".$ano."' and a.mes = '".$mes."' and a.quantidade >= '1' group by a.codfilial order by a.codfilial";
$result = execsql($sql);
while($row = mysql_fetch_row($result)){
echo '
	<td valign="top">
	<table width="230" border="0" align="center" cellpadding="2" cellspacing="0">
	  <tr class="tdcabecalho">
		...
		$result2 = execsql("select a.codgrpcliente from $mysql_metavendedori_table a where a.ano = '".$ano."' and a.mes = '".$mes."' and a.quantidade >= '1' and codfilial = '$row[0]' group by a.codgrpcliente order by a.codgrpcliente");
		...
		echo '<tr class="tdsubcabecalho1"> 
				...
				$result3 = execsql("select a.codsupervisor from $mysql_metavendedori_table a, $mysql_vendedorcxi_table b where a.codvendedor = b.codvendedor and a.ano = '".$ano."' and a.mes = '".$mes."' and a.quantidade >= '1' and codfilial = '$row[0]' and codgrpcliente = '$row2[0]' group by a.codsupervisor order by a.codsupervisor");
				...
				echo '<tr class="tddetalhe1"> 
						...
						$result4 = execsql("select a.codvendedor from $mysql_metavendedori_table a, $mysql_vendedorcxi_table b where a.codvendedor = b.codvendedor and a.ano = '".$ano."' and a.mes = '".$mes."' and a.quantidade >= '1' and codfilial = '$row[0]' and codgrpcliente = '$row2[0]' and codsupervisor = '$row3[0]' group by a.codvendedor order by a.codvendedor");
...
function QntClienteVendedor($codvendedor,$data,$tipo)
{
	global $mysql_clientesi_table,$mysql_vendasi_table, $bonificacao;

	if ($tipo == "total") {
		$result = execsql("select count(*) from $mysql_clientesi_table where codvendedor = '".$codvendedor."'");
		$row = mysql_fetch_array($result);
	} elseif($tipo == "dia") { $i=1;
		$result = execsql("select codcliente from $mysql_vendasi_table where datafatura = '".data($data)."' and codvendedor = '".$codvendedor."' group by codcliente  and codtipofatura $bonificacao");
		while($row = mysql_fetch_array($result)){	$hoje[$i] = $row[0];	$i++;	} $i=1;
		$result2 = execsql("select codcliente from $mysql_vendasi_table where datafatura >= '".substr(data($data),0,6)."01' and datafatura < '".data($data)."' and codvendedor = '".$codvendedor."' group by codcliente  and codtipofatura $bonificacao");
		...
		$result = execsql("select count(codcliente) from $mysql_vendasi_table where datafatura >= '".substr(data($data),0,6)."01' and datafatura <= '".data($data)."' and codvendedor = '".$codvendedor."' group by codcliente  and codtipofatura $bonificacao");
...
function PositivacaoVendedor($codvendedor,$data)
{
	...
	$qntcliente = mysql_fetch_array(execsql("select count(*) from $mysql_clientesi_table where codvendedor = '".$codvendedor."'"));

	$result = execsql("
	select codcliente, codproduto, datafatura from $mysql_vendasi_table 
...
function EnviarFlashVendedor($codvendedor,$data)
{
global $mysql_metavendedori_table, $mysql_vendasi_table, $mysql_produtos_table, $bonificacao;
...
$ano = substr($data,6,4);
	...
	$i = 1;
	$totalrealdia = 0;
	$result = execsql("select distinct codproduto from $mysql_vendasi_table where codvendedor = '".$codvendedor."' and datafatura <= '".data($data)."' and datafatura >= '".$ano.$mes."01' and client = '150'  and codtipofatura $bonificacao and codfilial $filiais order by codproduto");
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
	$result = execsql("select codproduto from $mysql_metavendedori_table where codvendedor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codfilial $filiais");
	while($row = mysql_fetch_array($result)){	$vendas[$i] = $row[0];	$i++;	}
...
if (substr($codvendedor,1,2) >= '80') {
	...
	$row = mysql_fetch_row($result);
	$qnttot = '0'; $vlrtot = '0';
	$resultve = execsql("select codgrpcliente, codfilial, codvendedor, sum(quantidade), avg(precomedio) from $mysql_metavendedori_table where codsupervisor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'  group by codgrpcliente, codfilial, codvendedor");
	while($row2 = mysql_fetch_array($resultve)){
	$result2 = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_vendasi_table where codgrpcliente = '".$row2[0]."' and codfilial = '".$row2[1]."' and codvendedor = '".$row2[2]."' and datafatura like '".$ano.$mes."%' and client = '150' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
	$row2 = mysql_fetch_row($result2);
	...
	$row2[0] = $qnttot; $row2[1] = $vlrtot;
		...
		$result2 = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional), count(codcliente), if(sum(valorbruto+valordesconto+valoradicional) > 0,((sum(valorbruto+valordesconto+valoradicional-valoricms-despicms-valoricmssub-valoripi-valorpis-valorcofins-custoproduto*quantidade)/sum(valorbruto+valordesconto+valoradicional))*100),0) from $mysql_vendasi_table where codvendedor = '".$codvendedor."' and client = '150' and datafatura <= '".data($data)."' and datafatura >= '".$ano.$mes."01' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
		$pdvacm = execsql("SELECT count( codcliente ) FROM $mysql_vendasi_table where codvendedor = '".$codvendedor."' and datafatura like '".$ano.$mes."%' and client = '150' and codproduto = '".$valor."'  and codtipofatura $bonificacao group by codcliente");
		$row2 = mysql_fetch_row($result2);
		$row2[2] = mysql_num_rows($pdvacm);

		$result3 = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional), count(codcliente) from $mysql_vendasi_table where codvendedor = '".$codvendedor."' and client = '150' and datafatura = '".data($data)."' and codproduto = '".$valor."'  and codtipofatura $bonificacao");
		$pdvacm = execsql("SELECT count( codcliente ) FROM $mysql_vendasi_table where codvendedor = '".$codvendedor."' and datafatura  = '".data($data)."' and client = '150' and codproduto = '".$valor."' and codtipofatura $bonificacao group by codcliente");
		$row3 = mysql_fetch_row($result3);
...
$sqlrenta = execsql("SELECT if(sum(valorbruto+valordesconto+valoradicional) > 0,((sum(valorbruto+valordesconto+valoradicional-valoricms-despicms-valoricmssub-valoripi-valorpis-valorcofins-custoproduto*quantidade)/sum(valorbruto+valordesconto+valoradicional))*100),0) rentabilidade FROM $mysql_vendasi_table where datafatura like '".$ano.$mes."%' and client = '150' and codvendedor = '$codvendedor'  and codtipofatura $bonificacao");
...
{
global $mysql_metavendedori_table, $mysql_resumogerali_table, $mysql_produtos_table;
...
while($row5 = mysql_fetch_array($result5)){	$valor = $row5[0]; 
	...
	$resultado = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_resumogerali_table where codvendedor = '".$codvendedor."' and client $bases and mes = '".$mes."' and ano = '".$ano."'  and codproduto = '".$valor."'");
	$vendedor = mysql_fetch_array($resultado);
	$resultado = execsql("select sum(quantidade), avg(precomedio) from $mysql_metavendedori_table where codvendedor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'");
...
if ($vendedor[0] != '') {
  ...
  <td align="right"><font size="-3">'.number_format($porcentovalor,'2',',','.').'</font></td>
	...
	$result2 = execsql("select codgrpcliente, codfilial, codvendedor, sum(quantidade), avg(precomedio) from $mysql_metavendedori_table where codsupervisor = '".$codvendedor."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'  group by codgrpcliente, codfilial, codvendedor");
	while($row2 = mysql_fetch_array($result2)){
		$result3 = execsql("select sum(quantidade), sum(valorbruto+valordesconto+valoradicional) from $mysql_resumogerali_table where codcanal = '".$row2[0]."' and client $bases and codfilial = '".$row2[1]."' and codvendedor = '".$row2[2]."' and mes = '".$mes."' and ano = '".$ano."' and codproduto = '".$valor."'");
		$row3 = mysql_fetch_row($result3);
		$qntvendas = $row3[0] + $qntvendas;
		$vlrvendas = $row3[1] + $vlrvendas;
		if ($i % 2) { $cor = $cor1;} else { $cor = $cor2;}
...
function createSelectDistribuidor() {
	...
	$filial = PermissaoFinal("codfilial","in");
        ...
        $result=execsql("SELECT codcliente, nome FROM $mysql_clientesi_table where codgrpcli = '65' and codfilial $filial GROUP BY codcliente order by nome ");
...
function MostrarDistribuidor($codfilial) { 
	global $mysql_clientesi_table;
    $sql = "SELECT codcliente, nome FROM $mysql_clientesi_table where codgrpcli = '65' AND codcliente='".$codfilial."' GROUP BY codcliente order by nome";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
...
function createSelectCliente($codvendedor) {
    ...
    $result=execsql("SELECT codcliente, nome FROM $mysql_clientesi_table where codvendedor = '$codvendedor' GROUP BY codcliente order by nome ");
...
function createSelectConcorrentes($codconcorrente = "") {
    ...
    $result=execsql("SELECT codconcorrente, nome FROM $mysql_concorrentesi_table GROUP BY codconcorrente order by nome ");
...
function createSelectTipoColaborador($tipocolaborador = "") {
	global $mysql_tipocolaboradori_table;

    $result=execsql("SELECT tipocolaborador, nome FROM $mysql_tipocolaboradori_table order by tipocolaborador ");
...
function MostrarTipoColaborador($tipo) { 
	global $mysql_tipocolaboradori_table;
    $sql = "SELECT tipocolaborador, nome FROM $mysql_tipocolaboradori_table where tipocolaborador ='".$tipo."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
...
{
	global $mysql_metamotivoi_table;

	$sql = "select idmetamotivo, nome from $mysql_metamotivoi_table order by nome";
	$result = execsql($sql);
...
{
	global $mysql_pvicmsi_table;

	$result = execsql("select idicms, valor from $mysql_pvicmsi_table order by idicms");
...
function MostrarEmb($codigo)
{
	global $mysql_grupo_embi_table;

	$sql = "select codigo, descricao from $mysql_grupo_embi_table where codigo = '".$codigo."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
