...
{
	global $mysql_empresa_table, $info;
  	$sql = "select emp codfilial, nome from $mysql_empresa_table order by emp asc";
	$result = execsql($sql);
...
function createSelectClasse()
{
	global $mysql_classe_table, $info;
  	$sql = "select codigo orgao, classe from $mysql_classe_table order by codigo asc";
	$result = execsql($sql);

	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
			if($info['orgao'] == $row[0]) echo $info['orgao']." selected"; 
				echo "> $row[1] </option>";
	}
}

function createSelectSituacao()
{
	global $mysql_status_table, $info;
  	$sql = "select codigo situacao, descricao from $mysql_status_table order by codigo asc";
	$result = execsql($sql);

	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
			if($info['situacao'] == $row[0]) echo $info['situacao']." selected"; 
				echo "> $row[1] </option>";
	}
}

function createSelectComarca()
{
	global $mysql_comarca_table, $info;
  	$sql = "select codigo comarca, descricao from $mysql_comarca_table order by descricao asc";
	$result = execsql($sql);

	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
			if($info['comarca'] == $row[0]) echo $info['comarca']." selected"; 
				echo "> $row[1] </option>";
	}
}
...
function getCheckPessoasProcessos($id)
{
    ...
    if ((mysql_num_rows($temparte) > 0) || (mysql_num_rows($tempatrono) > 0)) {
      ...
      if (mysql_num_rows($tempatrono) > 0) {
         ...
         echo "<table bgcolor=F5F5F5 cellSpacing=1 cellPadding=2 width=30% align=center border=0><tr><td class=tdcabecalho1 colspan=100%>";
         echo "É Advogado dos processos:</td></tr>";
               echo "<tr><td width=70% class=tdsubcabecalho1 align=left>Processo </td><td width=30% class=tdsubcabecalho1 align=center>Ação</td></tr>";
...
function getPessoasPPP($s, $pagina, $asc, $procura)
{
	...
	$sql = "select codpessoa, a.nome, patrono nome from $mysql_pessoas_table a ";
...
//SWITCH DE ORDEM (MODO)
	switch($s){
		...
		break;
		case ("codpessoa"):
			$sql .= " order by codpessoa $asc, a.nome";
			$gifb = $gif;
		break;
		default:
			$sql .= " order by a.nome $asc";
...
if (!$pagina) {
	...
	$result = execsql($sql .' LIMIT '.$inicio.' , '.$total_reg);
			...
			echo ' <tr>
                   ...
                   <td width=15% nowrap class=tdcabecalho1 align="center">Tipo </a></td>
       ...
       $ctrlclasse=1;
    	...
    	echo "<tr>
				...
				<td class=$classe>";
					echo $row[1] . " &nbsp;</td>
				<td class=$classe align=center>";
					echo $row[2]==1 ? "Advogado" : "Parte"; echo "</td>
   				<td class=$classe align=center>";
...
function listComarca()
{

	global $mysql_comarca_table;

	$sql = "select * from $mysql_comarca_table order by descricao asc";
	$result = execsql($sql);
	$num_rows = mysql_num_rows($result);
	...
	if($num_rows != 0){
		...
		while($row = mysql_fetch_row($result)){
			...
			echo "<tr><td colspan=100% align=center>";
			echo "<input type=text name=dsc_ta$i value=\"$row[1]\" size=\"70\">";
			echo "&nbsp;&nbsp;<a href=acomarca.php?m=delete&cod_ta=$row[0]><img border=0 src='../images/editdelete.gif'></a>";
			echo "</td>";
			echo "</tr>";
			$i++;
		}
...
function getNumComarca()
{
	global $mysql_comarca_table;

	$sql = "select count(codigo) from $mysql_comarca_table";
	$result = execsql($sql);
	$total = mysql_fetch_row($result);
	return $total[0];
...
function getNumTipoMovimentacao()
{
	...
	$sql = "select count(codigo) from $mysql_tipomovimentacao_table";
	$result = execsql($sql);
	$total = mysql_fetch_row($result);
	return $total[0];
...
function createSelectTipoAcao()
{
	...
	$sql = "select codtipoacao codtipoacao, descricao from $mysql_tipoacao_table order by codtipoacao asc";
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
...
function createSelectPartes()
{
	...
	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
			if($info['codpessoa'] == $row[0]) echo $info['nome']." selected"; 
				echo "> $row[1] </option>";
	}
...
**		Takes no arguments.  Creates the drop down menu for the list of platforms.
************************************************************************************************************/
function createSelectParte()
{
	global $mysql_ugrupos_table, $mysql_pessoas_table;

	$sql = "select codpessoa, nome from $mysql_pessoas_table  order by nome asc";
	$result = execsql($sql);
...
function createSelectTipoMovimentacao()
{
	...
	$sql = "select codigo cod, descricao from $mysql_tipomovimentacao_table order by descricao asc";
	$result = execsql($sql);

	while($row = mysql_fetch_row($result)){
			echo "<option value=\"$row[0]\"";
			if($info['cod'] == $row[0]) echo $info['cod']." selected"; 
				echo "> $row[1] </option>";
	}
				...
				<TABLE cellSpacing=1 cellPadding=2 width=100% border=0>
					...
					<TD class=tdcabecalho1 colspan=100% align=left><B>Movimentar Processo				</td>
   					...
   					<tr>
							...
							<td class=back><select name=codtipomov>';createSelectTipoMovimentacao();echo'</select>
							</td></tr>';
					/*		<td class=tdsubcabecalho1 align=right valign=top>Data da Movimentação</td>
							<td class=back><input type="text" name="datamov" value=$dthoje size="11" maxlength="10" onFocus="javascript:vDateType=\'3\'" onKeyUp="DateFormat(this,this.value,event,false,\'3\')" onBlur="DateFormat(this,this.value,event,true,\'3\')" value=""></tr><tr> */
     echo '<tr>
							<td class=tdsubcabecalho1 align=right valign=top>Data de Vencimentos</td>
							<td class=back><input type="text" name="datavenc" size="11" maxlength="10" onFocus="javascript:vDateType=\'3\'" onKeyUp="DateFormat(this,this.value,event,false,\'3\')" onBlur="DateFormat(this,this.value,event,true,\'3\')" value="">
						    <td class=tdsubcabecalho1 align=right valign=top>Hora</td>

							<td class=back><input type="text"  size="11" maxlength="10" name=hora></select>

				  </tr>
				  <tr>
							<td width=20% class=tdsubcabecalho1 align=right>Valor:</td>
							<td class=back width=30%> <input type=text size=20 name=valor>	</td>
							<td width=20% class=tdsubcabecalho1 align=right>Status:</td>
							<td class=back width=30%> <input type=radio name=status value=1>Concluído<input type=radio name=status value=0>Pendente</td>
                  </tr>
				  <tr>
...
if ($s=="valor")         { $corfob="black"; $bgfub="tdsubcabecalho1"; 	$gifb = $gif;} else  {$corfob="white"; $bgfub="tdcabecalho1";}
...
if (!$pagina) {
    ...
    echo '<TABLE bgcolor=F5F5F5 cellSpacing=0 cellPadding=0 width=90% align=center border=0>
			...
			echo ' <tr>
	               ...
	               <td width=35% nowrap class='.$bgfub.' align="center"><a href=?&asc='.$asc.'&s=valor&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfob.'>Valor </font></a> '.$gifb.'</td>
...
if (!isset($s))          { $s="codpessoa";}
if ($s=="numero")        { $corfoa="black"; $bgfua="tdsubcabecalho1"; 	$gifa = $gif;} else  {$corfoa="white"; $bgfua="tdcabecalho1";}
if ($s=="orgao")         { $corfob="black"; $bgfub="tdsubcabecalho1"; 	$gifb = $gif;} else  {$corfob="white"; $bgfub="tdcabecalho1";}
if ($s=="codtipoacao")   { $corfoc="black"; $bgfuc="tdsubcabecalho1"; 	$gifc = $gif;} else  {$corfoc="white"; $bgfuc="tdcabecalho1";}
if ($s=="datprocesso")   { $corfod="black"; $bgfud="tdsubcabecalho1"; 	$gifd = $gif;} else  {$corfod="white"; $bgfud="tdcabecalho1";}
...
if (!$pagina) {
	...
	$result = execsql($sql .' LIMIT '.$inicio.' , '.$total_reg);
                               ...
                               echo "<center><font size=1px></font>
...
";  if ($print==0) {echo" <p style='font-family: Verdana; font-size: 10px;'><B>[</B>Mostrar <input style='font-family: Verdana; font-size: 9px;' type=text name=tds size=2 value=$total_reg> processos por pagina.<B>]</B></p>";}
			...
			echo ' <tr>
	               ...
	               <td '.$borda.' nowrap class='.$bgfub.' align="center">';if ($print==0) { echo '<a href=?&asc='.$asc.'&s=orgao&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'&tds='.$total_reg.'><font color='.$corfob.'>';} echo 'Classe </font></a> '.$gifb.'</td>
	               <td '.$borda.' nowrap class='.$bgfuc.' align="center">';if ($print==0) { echo '<a href=?&asc='.$asc.'&s=codtipoacao&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'&tds='.$total_reg.'><font color='.$corfoc.'>';} echo 'Tipo </font></a> '.$gifc.'</td>
	               <td '.$borda.' nowrap class="tdcabecalho1" align="center">Advogado</a></td>
...
//JOGAR DADOS NA TELA
 	while($row = mysql_fetch_array($result)){
		...
		$sql = "select b.nome from $mysql_processopartes_table a, $mysql_pessoas_table b where a.codprocesso = '$row[4]' and a.codpessoa != '00000' and a.codpessoa = b.codpessoa order by b.nome";
		$ae = execsql($sql);
...
function getNomTipoMov($codigo)
{
	global $mysql_tipomovimentacao_table;
	$sql = "select descricao from $mysql_tipomovimentacao_table where codigo = '$codigo'";
	$result = execsql($sql);
	$total = mysql_fetch_row($result);
	return $total[0];
...
function getNomClasse($codigo)
{
	global $mysql_classe_table;
	$sql = "select classe from $mysql_classe_table where codigo = '$codigo'";
	$result = execsql($sql);
	$total = mysql_fetch_row($result);
	return $total[0];
...
function getNomEmpresa($emp)
{
	global $mysql_empresa_table;
	$sql = "select nome from $mysql_empresa_table where emp = '$emp'";
	$result = execsql($sql);
	$nome = mysql_fetch_row($result);
	return $nome[0];

}
function getComarca($comarca)
{
	global $mysql_comarca_table;
	$sql = "select descricao from $mysql_comarca_table where codigo = '$comarca'";
	$result = execsql($sql);
	$nome = mysql_fetch_row($result);
	return $nome[0];
...
function getSituacao($situacao)
{
	...
	$sql = "select descricao from $mysql_status_table where codigo = '$situacao'";
	$result = execsql($sql);
	$nome = mysql_fetch_row($result);
	return $nome[0];
