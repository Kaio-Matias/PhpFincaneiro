...
function createSelectLocal()
{
	global $mysql_centros_table, $info;
  	$sql = "select centro local, nome from gvendas.centros where centro ='1008' order by centro asc";
	$result = execsql($sql);
...
function  createSelectTpbaixa()
{
	global $mysql_tpbaixa_table, $info;
  	$sql = "select tp_baixa local, descricao from $mysql_tpbaixa_table";
	$result = execsql($sql);
