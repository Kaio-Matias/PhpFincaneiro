...
function getPessoasPPP($s, $pagina, $asc, $procura)
{
	...
	$sql = "select codpessoa, nome, patrono nome from $mysql_pessoas_table ";
       ...
       $ctrlclasse=1;
    	...
    	echo "<tr>
				...
				<td class=$classe>";
					echo $row[1] . " &nbsp;</td>
				<td class=$classe align=center>";
					echo $row[2]==1 ? "Patrono" : "Parte"; echo "</td>
   				<td class=$classe align=center>";
...
if (!$pagina) {
	...
	$result = execsql($sql .' LIMIT '.$inicio.' , '.$total_reg);
			...
			echo ' <tr>
	               ...
	               <td width=35% nowrap class='.$bgfub.' align="center"><a href=?&asc='.$asc.'&s=tribunal&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfob.'>Tribunal </font></a> '.$gifb.'</td>
...
";  if ($print==0) {echo" <p style='font-family: Verdana; font-size: 10px;'><B>[</B>Mostrar <input style='font-family: Verdana; font-size: 9px;' type=text name=tds size=2 value=$total_reg> processos por pagina.<B>]</B></p>";}
			...
			echo ' <tr>
	               ...
	               <td '.$borda.' nowrap class='.$bgfub.' align="center">';if ($print==0) { echo '<a href=?&asc='.$asc.'&s=tribunal&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'&tds='.$total_reg.'><font color='.$corfob.'>';} echo 'Tribunal </font></a> '.$gifb.'</td>
	               ...
	               <td '.$borda.' nowrap class="tdcabecalho1" align="center">Partes</a></td>
