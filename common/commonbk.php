...
**	arquivo:	common.php
**
**	Ete arquivo contem as variaveis do sistema e todas as funções q são utilizadas na intranet
**
************************************************************************************************************
	**
	**	author:	Saulo Felipe
...
$users_limit = $var['users_per'];					// Número de usuários a serem listados
$helpdesk_name = $var['name'];						// Nome da Intranet
$admin_email = $var['admin_email'];					// Email do Administrador da Intranet
$site_url = $var['site_url'];						// URL da página principal
...
$version = $var['version'];							// Versão da Intranet
				...
				<td> 
					<table cellSpacing=1 cellPadding=5 width="100%" border=0>
						<tr> 
							...
							<td width=20% class=back2 align=center><b>Usuário</b></td>
...
function printSuccess($msg)
{
	echo '<TABLE class=border cellSpacing=0 cellPadding=0 width="100%" align=center border=0>
			...
			<TABLE cellSpacing=1 cellPadding=5 width="100%" border=0>
				<TR> 
				<TD class=info align=middle><B>Operação efetuada com sucesso!</B></TD>
				</TR>
...
function createLiberarMenu($flag = 0)
{
	...
	if($flag == 1) {
		echo "<select name=liberar><option value=1>Sim</option><option value=0 selected>Não</option></select>";
	} else {
...
function createCorporativoMenu($flag = 0)
{
	...
	if($flag == 1) {
		echo "<select name=corporativo><option value=1>Sim</option><option value=0>Não</option></select>";
	} else {
...
{	
	...
	printError("<center><br>
					  <b>Você não tem permissão</b><br>
					  <b>Usuário:</b> $cookie_name<br>
				      <b>Transação:</b> $transacao<br>
					  <b>Descrição da Transação:</b> $row[0]<br>
...
**		Seleciona usuários pelo tipo de grupo 
************************************************************************************************************/
function getUserTipoGrupo($tg, $order, $offset, $t)
{
	...
	//print out the html crap...this is ugly.
		...
		if($offset < getTotalUserTipoGrupo($tg)){
			echo "&nbsp;<a href=index.php?t=umodif&o=$o&offset=$offset>Próximo</a>";
		}
		else{
			echo "&nbsp;Próximo";
		}
...
**		Retorna de acordo com o tipo e campo escolhido a estrutura organizacional do usuário.
**		Obs.: $cookie_name -> login do usuário.
**
**		Entradas: $campo -> Campo que a função ira veirificar na estrutura organizacional.
**				  $tipo	 -> Tipo de ação: retornar matriz ou os valores para a pesquisa do campo em uma
**						    tabela
**				  $aplicacao -> Nome da aplicação que a permissão esta relacionada. (Padrão = "GESTVENDAS")
**
**	  	   Saída: A função retorna para uma variavel ou uma matriz.
**
************************************************************************************************************/
...
**		Retorna o código e o nome do centro.
**
**		Entradas: $codcentro -> Valor do centro para ser monstada a query.
**
**	  	   Saída: A função retorna com a utilização do return a string com o codigo do centro e o nome
**
************************************************************************************************************/
...
function MostrarCentro($codcentro)
{
	...
	$sql = "select centro, nome from $mysql_centros_table where centro = '".$codcentro."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
