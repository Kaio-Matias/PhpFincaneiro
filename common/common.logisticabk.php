...
{
	global $mysql_frete_table, $mysql_romaneios_table, $mysql_baixas_table, $mysql_romfreteposto_table, $mysql_romfreteextra_table;
...
<?
	$result = execsql("select a.notafiscal, a.codcliente, a.devolucao, b.meiopg, sum(valorproduto), sum(valorfreteproduto), sum(pesoproduto), a.tipofatura, c.cidade
		from $mysql_frete_table a, $mysql_romaneios_table c
		...
		if ($nf == $row[0]) {
			echo '
			 <tr class="tdsubcabecalho1">
			   ...
			   <td align="center" colspan=3><font size="1"></td>
 			   <td align="center"><font size="1">Valor</td>
 			   <td align="center"><font size="1">Frete</td>
			   <td align="center"><font size="1">Peso</td>
			 </tr>';
			$result2 = execsql("select a.codproduto, sum(valorproduto),sum(valorfreteproduto), sum(pesoproduto)
				from $mysql_frete_table a
				where a.notafiscal = '$nf' group by codproduto order by codproduto");
