...
$producao_name = "Controle de Produção - Valedourado";
...
function createSelectProdutosMeta($produto="",$data)
{
	...
	global $mysql_produtos_table, $mysql_meta_producao_table, $mysql_produtostemp_table ;
	...
	$result = execsql("select distinct codproduto, nome from $mysql_produtostemp_table $where order by codproduto");
	while($row = mysql_fetch_row($result)) {
		if ($produto == $row[0]) $select2 = " selected"; else $select2 = " ";
		echo "<option value=\"$row[0]\" $select2>".$row[0]." - ".$row[1]."</option>";
	}
...
function MostrargrpProduto($codgrpproduto)
{
	...
	$sql = "select nome from gvendas.grupoproduto where codgrpproduto = '".$codgrpproduto."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0];
}
