...
function createSelectCentro()
{
	global $mysql_centros_table, $info;
  	$sql = "select centro codfilial, nome from $mysql_centros_table order by nome asc";
	$result = execsql($sql);
...
if ($s=="centro")        { $corfoc="black"; $bgfuc="tdsubcabecalho1"; 	$gifc = $gif;} else  {$corfoc="white"; $bgfuc="tdcabecalho1";}
if ($s=="valor")         { $corfod="black"; $bgfud="tdsubcabecalho1"; 	$gifd = $gif;} else  {$corfod="white"; $bgfud="tdcabecalho1";}
...
if (!$pagina) {
    ...
    echo '<TABLE bgcolor=F5F5F5 cellSpacing=0 cellPadding=0 width=90% align=center border=0>
			...
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>';
	             echo ' <tr>
	               <td width=20% nowrap class='.$bgfua.' align="center"><a href=?&asc='.$asc.'&s=numero&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfoa.'>Contrato Nº</font></a> '.$gifa.'</td>
	               ...
	               <td width=33% nowrap class='.$bgfuc.' align="center"><a href=?&asc='.$asc.'&s=centro&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfoc.'>Centro </font></a> '.$gifc.'</td>
	               <td width=12% nowrap class='.$bgfud.' align="center"><a href=?&asc='.$asc.'&s=valor&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfod.'>Valor </font></a> '.$gifd.'</td>
                                      </tr>
                   ';
//JOGAR DADOS NA TELA
 	while($row = mysql_fetch_array($result)){
...
/***********************************************************************************************************
**	function MostrarCategoria($codtipoacao):
**		Mostra a Catgoria de contrato
************************************************************************************************************/
function Mostrartpacao($codtipoacao)
{
	$sql = "select codtipoacao , descricao from contrato.tipoacao where codtipoacao  = '".$codtipoacao ."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".ucfirst(strtolower($row[1]));
}
...
function MostrarTipoacao($codtipoacao)
{
	global $mysql_tipoacao_table;

	$sql = "select codtipoacao, descricao from $mysql_tipoacao_table where codtipoacao = '".$codtipoacao."'";
	$result = execsql($sql);
	$row = mysql_fetch_row($result);
	return $row[0]." - ".$row[1];
}
