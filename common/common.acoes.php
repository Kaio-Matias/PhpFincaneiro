...
/***********************************************************************************************************
**	function createSelectCliente():
************************************************************************************************************/
/*
function createSelectCliente($codcliente) {
	global $mysql_clientes_table;

	$filial = PermissaoFinal("codfilial","in");
      //$result=execsql("SELECT codcliente, nome FROM $mysql_clientes_table where codfilial $filial GROUP BY codcliente order by nome ");

        $result=execsql("SELECT codcliente, nome FROM $mysql_clientes_table ORDER BY codcliente order by nome ");
...
function createSelectCliente() {
	...
	$sql = "SELECT codcliente, nome FROM $mysql_clientes_table where codfilial $filial ORDER BY codcliente";
	$result = execsql($sql);
	while($row = mysql_fetch_row($result)){
		if ($produto == $row[0]) { $select = "selected"; } else { $select = ""; };
			echo "<option value=\"$row[1]\" $select>".$row[0] ." - ". $row[1];
	}
echo "</select>";
}
...
if (!isset($s))          { $s="codprocesso";}
if ($s=="codprocesso")   { $corfoa="black"; $bgfua="tdsubcabecalho1"; 	$gifa = $gif;} else  {$corfoa="white"; $bgfua="tdcabecalho1";}
if ($s=="codcliente")    { $corfob="black"; $bgfub="tdsubcabecalho1"; 	$gifb = $gif;} else  {$corfob="white"; $bgfub="tdcabecalho1";}
...
$result = execsql($sql .' LIMIT '.$inicio.' , '.$total_reg);
            ...
            echo "<center><font size=1px><b> Foram encontradas $tr ações Mostrando página ($pc) de (".ceil($tp).") </b></font></center><br><br>";
          	echo '<TABLE bgcolor=F5F5F5 cellSpacing=0 cellPadding=0 width=90% align=center border=0>
			<TR>
			<TD>
				<TABLE cellSpacing=1 cellPadding=1 width="100%" border=0>';
			echo ' <tr>
	               <td width=20% nowrap class='.$bgfua.' align="center"><a href=?&asc='.$asc.'&s=codprocesso&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfoa.'>Numero </font></a> '.$gifa.'</td>
	               <td width=35% nowrap class='.$bgfub.' align="center"><a href=?&asc='.$asc.'&s=codcliente&pback='.$pbackx.'&stmt='.$sql2.'&pagina='.$pagina.'><font color='.$corfob.'>Cliente </font></a> '.$gifb.'</td>
...
//JOGAR DADOS NA TELA
    ...
    case 0:
       ...
       $ctrlclasse=1;
    	...
    	echo "<tr>
            	<td class=$classe align='center'> <a href=\"$pbackx.php?id=" . $row[0] . "\">";
    			echo $row[0] . "</a> &nbsp;</td>
            	<td class=$classe align='center'> <a href=\"$pbackx.php?id=" . $row[0] . "\">";
    			echo $row[1] . "</a> &nbsp;</td>
            	<td class=$classe align='center'> <a href=\"$pbackx.php?id=" . $row[0] . "\">";
    			echo getNomTipoAcao($row[3]) . "</a> &nbsp;</td>
            	<td class=$classe align='center'> <a href=\"$pbackx.php?id=" . $row[0] . "\">";
    			echo dataphp($row[2]) . "</a> &nbsp;</td>
             </tr>";
	}
echo '</table><br><Br><table class=tddetalhe1 align=center><tr><td>' ;
